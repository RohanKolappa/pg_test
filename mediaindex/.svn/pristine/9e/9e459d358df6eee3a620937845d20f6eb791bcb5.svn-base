...
if [ x$ip = "x" -o x$base = "x" ] ; then
    ...
    echo "libmediaindex.so library base offset on the command line."
    echo
    echo "For example for a segfault message such as:"
    echo "segfault at 0 ip b7689cc5 sp b50fe090 error 4 in libmediaindex.so[b7675000+40000]"
    echo 
...
# Create full objectdump of the libmediaindex.so file
skip=$3
if [ x$skip != "xskip" ] ; then
   echo "Extracting debug info from  libmediaindex.so"
   echo "This will take a while..."
   objdump -DClS libmediaindex.so >| LIBMEDIAINDEX_DEBUG
fi
...
grep -n -A 20 -B 20 "${diff}:" LIBMEDIAINDEX_DEBUG
