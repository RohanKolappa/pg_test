...
public class ExternalComponent implements Component {
	private static Logger logger = Logger.getLogger(ExternalComponent.class);
    /**
    ...
    private Map<String, IQResultListener> resultListeners = new ConcurrentHashMap<String, IQResultListener>(8, 0.9f, 1);
    private Map<String, Long> resultTimeout = new ConcurrentHashMap<String, Long>(8, 0.9f, 1);
    ...
    public ExternalComponent(Component component, ExternalComponentManager manager, int maxThreads) {
        ...
        readerThreadPool = new ThreadPoolExecutor(maxThreads, maxThreads, 15, TimeUnit.SECONDS,
                new LinkedBlockingQueue<Runnable>(), new ThreadPoolExecutor.CallerRunsPolicy());
            ...
            public void run() {
                if (packet instanceof IQ) {
                    ...
                    if (IQ.Type.result == iqType || IQ.Type.error == iqType) {
                    	...
                    	logger.info("### ExternalComponent: Removing from cache");
    ...
    public void send(Packet packet) {
        synchronized (writer) {
            try {            	
                ...
                // Log the exception
            	logger.info("$$$$$$$$$$$$$$$$ ERRROR IN SENDING PACKET $$$$$$$$$$$$$$$$$$$$$$$");
            	logger.error("SEND",e);
                manager.getLog().error(e);
                if (!shutdown) {
                	logger.info("$$$$$$$$$$$$$$$$ ERRROR IN SENDING PACKET SHUTDOWN $$$$$$$$$$$$$$$$$$");
    ...
    public void start() {
        ...
        readerThreadPool.execute(readerThread);
        //readerThread.start();
     ...
     //   boolean isConnected = false;
       ...
       /* while (!isConnected && !shutdown) {
            try {
                ...
                // connection
                if (shutdown) {
                	logger.info("##### Reconnecting: Disconnect ####");
                    disconnect();
                }
                else {
                    // Component is back again working so start it up again
                	logger.info("##### Reconnecting: Start ####");
                    start();
                }
            } catch (ComponentException e) {
                logger.error("Error trying to reconnect with the server", e);
