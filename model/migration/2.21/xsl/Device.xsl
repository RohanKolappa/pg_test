<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//DeviceStatus">
        ...
        <xsl:choose>
            <xsl:when test='"2.22"=$objectXml_serviceVer'>
	...
	<xsl:template mode="after" match="//GenlockMode">
		<xsl:choose>
			<xsl:when test="../MasterDelay">
			</xsl:when>
			<xsl:otherwise>
				<MasterDelay>2</MasterDelay>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
     
	<xsl:template match="//GenlockMode">
		<xsl:variable name="rootname" select="name(/*)"/>
		<xsl:choose>
			<xsl:when test=".='' and $rootname='V2D-Rx-Device' ">
				<GenlockMode>Master</GenlockMode>
			</xsl:when>
			<xsl:when test=".=''">
				<GenlockMode>None</GenlockMode>
			</xsl:when>
			<xsl:otherwise>
