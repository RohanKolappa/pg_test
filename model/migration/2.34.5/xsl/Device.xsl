<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//DeviceStatus[not(*)]">
        ...
        <xsl:choose>
            <xsl:when test='"2.34.6"=$objectXml_serviceVer'>
    ...
    <xsl:template mode="append" match="//SystemElementsConfig">
	<xsl:choose>
		<xsl:when test="SystemLogConfig">
		</xsl:when>
		<xsl:otherwise>
			<SystemLogConfig Dirty="CLEAN">
				<Level>INFO</Level>
			</SystemLogConfig>
		</xsl:otherwise>
	</xsl:choose>
...
<xsl:template mode="append" match="//SystemStore">
	<xsl:choose>
		<xsl:when test="LogsReservation">
		</xsl:when>
		<xsl:otherwise>
			<LogsReservation Size="5368709120">
				<RootDirPath>/data1/logs/</RootDirPath>
			</LogsReservation>
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
