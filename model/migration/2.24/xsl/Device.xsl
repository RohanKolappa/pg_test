<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//DeviceStatus">
        ...
        <xsl:choose>
            <xsl:when test='"2.25"=$objectXml_serviceVer'>
    ...
    <!-- copy device status only if serviceVer is of last migration -->
    <xsl:template match="//IPVSClient-Device//IPVSClientDisplayConfig/Display" mode="add-atts">
	   <xsl:attribute name="hres" />
      <xsl:attribute name="vres" />
 	</xsl:template>
	<xsl:template match="//IPVSClient-Device//IPVSClientDisplayConfig/Display" mode="append">
	   <Panel id="" enable="" content="">
          <Geometry zIndex="" border="" vpos="" hpos="" vres="" hres="" aspect="" transparency=""></Geometry>
      </Panel>
	</xsl:template>
	<xsl:template match="//IPVSClient-Device//IPVSClientDisplayConfig/Canvas" mode="add-atts">
