<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- copy device status only if serviceVer is of last migration -->
     <xsl:template match="//DeviceStatus[not(*)]">
        ...
        <xsl:choose>
            <xsl:when test='"2.28.8"=$objectXml_serviceVer'>
	...
	<xsl:template match="//XMPPServiceConfig/XMPPServer" mode="after">
		<ServicePreferences>
        	<Login>
            	<MaxMediaClients></MaxMediaClients>
           	</Login>
        </ServicePreferences>
	</xsl:template>	 	
