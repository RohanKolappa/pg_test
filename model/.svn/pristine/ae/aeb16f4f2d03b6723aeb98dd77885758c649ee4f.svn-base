<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//DeviceStatus[not(*)]">
        ...
        <xsl:choose>
            <xsl:when test='"2.33.9"=$objectXml_serviceVer'>
    ...
    <xsl:template mode="append" match="//SystemElementsStatus">
		<xsl:choose>
			<xsl:when test="IdentifyStatus">
			</xsl:when>
			<xsl:otherwise>
				<IdentifyStatus>
					<Parameters>
						<Parameter name="identifying"></Parameter>
					</Parameters>
				</IdentifyStatus>
			</xsl:otherwise>
		</xsl:choose>
