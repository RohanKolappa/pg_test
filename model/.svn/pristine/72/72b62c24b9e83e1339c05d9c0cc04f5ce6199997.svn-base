<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//DeviceStatus[not(*)]">
        ...
        <xsl:choose>
            <xsl:when test='"2.34.11"=$objectXml_serviceVer'>
	...
	<xsl:template mode="append" match="//MS-Device/*/ServiceElementsConfig">
	
	    <xsl:if test="//MS-Device or //MS-25-Device or //MS-25-Device" >
	    <xsl:choose>
			<xsl:when test="CentralLoggingClientServiceConfig">
			</xsl:when>
			<xsl:otherwise>
				<CentralLoggingClientServiceConfig
					Dirty="CLEAN">
					<CentralLoggingClientEnable>false</CentralLoggingClientEnable>
 		 ...
 		 <xsl:choose>
			<xsl:when test="BarcoBusClientServiceConfig">
			</xsl:when>
			<xsl:otherwise>
				<BarcoBusClientServiceConfig Dirty="CLEAN">
					<BarcoBusClientEnable>false</BarcoBusClientEnable>
					<ServerPath></ServerPath>
					<Username></Username>
					<Password></Password>
					<Queue></Queue>
				</BarcoBusClientServiceConfig>
			</xsl:otherwise>
		</xsl:choose>
