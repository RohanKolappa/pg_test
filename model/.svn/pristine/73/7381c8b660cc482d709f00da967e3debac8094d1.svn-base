<xsl:stylesheet version="1.0"
	...
	<xsl:template match="@Ver">
		<xsl:attribute name="Ver">2.6</xsl:attribute>
	</xsl:template>
	<xsl:variable name="portType" select="//MediaStreamIOPortResource/Info/Type" />
	...
	<xsl:template
		match="/MediaStreamIOPortResource/Info/StreamTypeList/StreamType">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<StreamType>
					<xsl:choose>
						<xsl:when test="$portType='StreamSrc'">
							<Out>
								<xsl:choose>
