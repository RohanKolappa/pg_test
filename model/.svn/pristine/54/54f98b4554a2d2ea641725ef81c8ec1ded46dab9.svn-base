<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template mode="append" match="//V2D-XPi-Device/DeviceConfig/AVCodecElementsConfig/XP200PortConfig">
		  <xsl:copy-of select="//DeviceConfig/V2DEncoderSyncConfig">
		  </xsl:copy-of>
		  <xsl:copy-of select="//DeviceConfig/V2DEncoderLayoutConfig">
		  </xsl:copy-of>
	</xsl:template>
	<xsl:template match="//DeviceConfig/V2DEncoderSyncConfig">
	</xsl:template>
	<xsl:template match="//DeviceConfig/V2DEncoderLayoutConfig">
	</xsl:template>
    ...
    <xsl:template match="//DeviceStatus[not(*)]">
        ...
        <xsl:choose>
            <xsl:when test='"2.34.12"=$objectXml_serviceVer'>
