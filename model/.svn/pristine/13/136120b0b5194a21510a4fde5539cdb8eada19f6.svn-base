<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template mode="after" match="//XP200PortConfig/V2DCodecVideoLookupTable">
       <V2DDisplayOverrideTable Dirty="CLEAN">      
       </V2DDisplayOverrideTable>      
   </xsl:template>

   <xsl:template  match="//IFrameInterval">
        <xsl:choose>
            <xsl:when test="../EnableVideoOutput">
            </xsl:when>
            <xsl:otherwise>
   				<EnableVideoOutput>true</EnableVideoOutput>
            </xsl:otherwise>
        </xsl:choose>
   </xsl:template>
   
   <xsl:template mode="after" match="//GenlockMode">
        <xsl:choose>
            <xsl:when test="../EnableVideoOutput">
            </xsl:when>
            <xsl:otherwise>
   ...
   <xsl:template mode="add-atts" match="//XP200DisplayConfig">
    ...
    <xsl:template match="//DeviceStatus">
        ...
        <xsl:choose>
            <xsl:when test='"2.14"=$objectXml_serviceVer'>
