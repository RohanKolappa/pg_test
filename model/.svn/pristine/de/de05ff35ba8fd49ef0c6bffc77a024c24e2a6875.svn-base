<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//DeviceStatus[not(*)]">
        ...
        <xsl:choose>
            <xsl:when test='"2.34.3"=$objectXml_serviceVer'>
    ...
    <xsl:template mode="append"
		match="//EMS-Device/*/SystemElementsConfig/UpgradeConfig">	
            <xsl:choose>
 	...
 	<xsl:template mode="append"
		match="//OWS-NGP-200-Device/*/SystemElementsConfig/UpgradeConfig">	
            <xsl:choose>
			<xsl:when test="Source">
			</xsl:when>
			<xsl:otherwise>
 ...
 <xsl:template mode="append"
		match="//OWS-GENERIC-Device/*/SystemElementsConfig/UpgradeConfig">	
            <xsl:choose>
			<xsl:when test="Source">
			</xsl:when>
			<xsl:otherwise>
    ...
    <xsl:template mode="append"
		match="//OWS-NGP-100-Device/*/SystemElementsConfig/UpgradeConfig">	
            <xsl:choose>
			<xsl:when test="Source">
			</xsl:when>
			<xsl:otherwise>
