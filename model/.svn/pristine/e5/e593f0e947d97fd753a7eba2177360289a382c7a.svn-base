<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/*/DeviceConfig/NetworkElementsConfig/NATTraversalTableConfig">
        <NATZoneTableConfig Dirty="CLEAN">
            <NATZoneEntry Dirty="CLEAN" ListIndex="1">
                <Name/>
                <IPAddress/>
            </NATZoneEntry>
        </NATZoneTableConfig>
    ...
    <xsl:template mode="append" match="/V2D-XP200-Device/DeviceConfig/AVCodecElementsConfig/XP200PortConfig/XP200DisplayConfig">
        <xsl:choose>
            <xsl:when test="VerticalPosition">
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:choose>
            <xsl:when test="HorizontalPosition">
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:choose>
            <xsl:when test="VerticalOffset">
            </xsl:when>
            <xsl:otherwise>

                <xsl:choose>
                    <xsl:when test="VeriticalOffset">
                        <VerticalOffset>
                            <xsl:value-of select="VeriticalOffset"/>
                        </VerticalOffset>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="//DeviceStatus">
        ...
        <xsl:choose>
            <xsl:when test='"2.1"=$objectXml_serviceVer'>
