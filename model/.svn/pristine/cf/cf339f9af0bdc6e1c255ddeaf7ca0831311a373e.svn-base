<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//DeviceStatus[not(*)]">
        ...
        <xsl:choose>
            <xsl:when test='"2.34.0"=$objectXml_serviceVer'>
...
<xsl:template mode="append"
		match="*[starts-with(name(),'DMS') or starts-with(name(),'MS')]/*/SystemElementsConfig">
		
		<KeyboardConfig Dirty="CLEAN">
			<Layout></Layout>
			<Variant></Variant>
		</KeyboardConfig>
		<RegionalSettingsConfig Dirty="CLEAN">
			<Locale></Locale>
			<Timezone></Timezone>
		</RegionalSettingsConfig>
		<HTTPModeConfig Dirty="CLEAN">
			<Enable></Enable>
		</HTTPModeConfig>
	</xsl:template>
	<xsl:template 
		match="//HAServiceConfig">
		<xsl:copy>
