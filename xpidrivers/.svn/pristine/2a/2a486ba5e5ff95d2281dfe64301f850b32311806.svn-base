...
#define DRIVER_NAME "ptp"




#define XIo_Out32(address,Val) \
  iowrite32((Val), (address))

#define XIo_In32(address) \
  ioread32((address))

#define TRUE    (1)
#define FALSE   (0)

#define PTP_IRQ_NUMBER  (4)
...
struct ptp_device {
  ...
  /* DelayReq enable/disable
     Take into account the DelayReg timestamps when adjusting the rtc.
     This is only valid when an AVB capable switch is used. It measures the delay over the wire */
  uint32_t delayTimestampEnable;

  /* Check if the ptp hardware is stuck -- results in sending timestamps = 0 on the network and
	 resetting all slaves -> genlock no longer working... */
  uint32_t hangupDetectCounter;
  uint32_t oldSecondsCounter;
