//============================================================================
// 633 WinTest Code.
// show_packet,cpp: Ultra-simple 633 command-line communications example
...
void ShowReceivedPacket(void) 
  ...
  for(i=0;i<incoming_response.data_length;i++)
    {
    if(isprint(incoming_response.data[i]))
      {
      int
        current_length=strlen(expanded);
      expanded[current_length]=incoming_response.data[i];
      ...
      char
        bin_number[5];
      sprintf(bin_number,"\\%03d",incoming_response.data[i]);
      strcat(expanded,bin_number);
      }
    }

  //key
  if(incoming_response.command==0x80)
    printf("C=%d(key:%s),L=%d,D=\"%s\",CRC=0x%04X\n",
      incoming_response.command,incoming_response.data[0]<=20?
        key_names[incoming_response.data[0]]:
          "HUH?",
      incoming_response.data_length,
      expanded,
      incoming_response.CRC);
  else
	//any other packet types
    printf("C=%d(%s),L=%d,D=\"%s\",CRC=0x%04X\n",
      incoming_response.command,
      ((incoming_response.command&0xC0)==0xC0)?
        error_names[0x3F&incoming_response.command]:
          command_names[0x3F&incoming_response.command],
      incoming_response.data_length,
      expanded,
      incoming_response.CRC);
}
