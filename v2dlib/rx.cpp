...
int ParseArguments(int a_argc, char* a_argv[], struct Rx_Params &a_RxParams, struct Status & a_status) {
    ...
    while ((nOpt = pgetopt(a_argc, a_argv, (char *) "s:h:p:P:a:b:m:w:F:C:d:S:M:R:v:D:")) > 0) {
           switch (nOpt) {
            case 'a':
                if (atoi(poptarg))
                    a_RxParams.nAVOption = AVOPTION_AUDIO_VIDEO;
...
void SaveVideoFrameToFile(unsigned char *a_pDecodedBuffer,
    ...
    if (a_nSrcWidth == a_nSrcOrigWidth && a_nSrcHeight == a_nSrcOrigHeight) {
        oSaveBmp.SetBitsBGRA(a_pDecodedBuffer, a_nSrcWidth, a_nSrcHeight);
        oSaveBmp.Save(filename, 32);
    }
    else {
        // Need to crop out pixels on the bottom and right
        uint8_t * croppedBuffer = new uint8_t[a_nSrcOrigWidth * a_nSrcOrigHeight * 4];
        uint8_t * srcBufferLine = a_pDecodedBuffer;
        uint8_t * dstBufferLine = croppedBuffer;
        for (int j = 0; j < a_nSrcOrigHeight; j++) {
            memcpy(dstBufferLine, srcBufferLine, a_nSrcOrigWidth * 4);
            srcBufferLine += a_nSrcWidth * 4;
            dstBufferLine += a_nSrcOrigWidth * 4;
        }
        oSaveBmp.SetBitsBGRA(croppedBuffer, a_nSrcOrigWidth, a_nSrcOrigHeight);
        oSaveBmp.Save(filename, 32);
