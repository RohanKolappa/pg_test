...
public class ExternalComponent implements Component {
    ...
    public ExternalComponent(Component component, ExternalComponentManager manager, int maxThreads) {
        ...
        threadPool = new ThreadPoolExecutor(maxThreads, maxThreads, 15, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new ThreadPoolExecutor.CallerRunsPolicy());
    }
    ...
    public void send(String to, String packet) {
        synchronized (writer) {
            try {
                ...
                // Log the exception
                if(packet !=null){
                    manager.getLog().error("ERROR Connection was lost: "+packet, e); 
                }else{
                    manager.getLog().error("ERROR Connection was lost: ", e);
                }
                if (!shutdown) {
    ...
    public void send(Packet packet) {
        synchronized (writer) {
            try {
                ...
                // Log the exception
                if(packet !=null){
                    manager.getLog().error("ERROR Connection was lost: "+packet.toXML(), e); 
                }else{
                    manager.getLog().error("ERROR Connection was lost: ", e);
                }
                if (!shutdown) {
