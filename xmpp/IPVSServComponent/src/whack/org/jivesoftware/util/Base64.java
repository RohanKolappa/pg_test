...
public class Base64 {
    ...
    private final static void usage(String msg) {
        MRLog.error(msg);
        MRLog.error("Usage: java Base64 -e|-d inputfile outputfile");
    } // end usage
    ...
    private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
        ...
        // Example: DkLE
            ...
            try {
                ...
                MRLog.debug("" + source[srcOffset] + ": " + (DECODABET[source[srcOffset]]));
                MRLog.debug("" + source[srcOffset + 1] + ": " + (DECODABET[source[srcOffset + 1]]));
                MRLog.debug("" + source[srcOffset + 2] + ": " + (DECODABET[source[srcOffset + 2]]));
                MRLog.debug("" + source[srcOffset + 3] + ": " + (DECODABET[source[srcOffset + 3]]));
                return -1;
            } // end catch
        }
    } // end decodeToBytes
    ...
    public static byte[] decode(byte[] source, int off, int len, int options) {
        ...
        for (i = off; i < off + len; i++) {
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) // White space, Equals sign or better
            {
                ...
                MRLog.error("Bad Base64 input character at " + i + ": " + source[i] + "(decimal)");
                return null;
            } // end else:
    ...
    public static byte[] decodeFromFile(String filename) {
        ...
        try {
            ...
            if (file.length() > Integer.MAX_VALUE) {
                MRLog.error("File is too big for this convenience method (" + file.length() + " bytes).");
                return null;
            ...
            MRLog.error("Error decoding from file " + filename);
        } // end catch: IOException
        finally {
            try {
                bis.close();
    ...
    public static String encodeFromFile(String filename) {
        ...
        try {
            ...
            MRLog.error("Error encoding from file " + filename);
        } // end catch: IOException
        finally {
            try {
                bis.close();
