...
public abstract class MediaFileBase implements DBActionManager {
    ...
    protected String getExportDBXMLPath(String exportDBXML) {
        ...
        file = new File(IpvsServGlobals.getHomeDirectory() + "/conf/xmls/mediafile/" + exportDBXML);
        if (file.exists()) {
            return file.getAbsolutePath();
        }
        return null;
    ...
    protected void ensureZipFolder(String zipFolderPath) throws Exception {
        String zipFolderPathExport = IpvsServGlobals.getProperty("admindbapi.mediafile_export", "/data/mediafile/export");
        String zipFolderPathImport = IpvsServGlobals.getProperty("admindbapi.mediafile_import", "/data/mediafile/import");
        ...
        if (getMediaDirZipType().equalsIgnoreCase("tar")) {
            file = new File(file, NID + ".tar");
        } else if (getMediaDirZipType().equalsIgnoreCase("tgz")) {
            file = new File(file, NID + ".tgz");
        } else {
            file = new File(file, NID + ".tgz");
        }
    ...
    public String getWorkarea_folder() {
        if(workarea_folder==null){
            String NID = Dom4jUtils.value(Dom4jUtils.element(requestEL, "params"), "mediaStreamFileResourceNID", "");
            type = Dom4jUtils.getAttributeValue(requestEL.getParent(), "type", "").toLowerCase();
            if(type.equalsIgnoreCase("export")) {
            	NID = Dom4jUtils.value(Dom4jUtils.element(requestEL, "params"), "exportMediaStreamFileResourceNID", "");
            }
            workarea_folder = workareaFolderPath(type, NID);
        }
    ...
    public String getDBIndexFilePath() {
        File dir = new File(getWorkarea_folder());
        File indexFile = new File(dir, "_index");
        return indexFile.getAbsolutePath();
    }
    public File getDBImportFolder2(){
