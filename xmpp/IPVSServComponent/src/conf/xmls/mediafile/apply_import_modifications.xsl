<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:param name="state_msg">Importing</xsl:param>
    ...
    <xsl:template match="/MediaStreamFileResource/Info/Properties/ParentMediaDirNID">
   		<xsl:choose>
            <xsl:when test="/MediaStreamFileResource[Info/MediaStoreFileInfo/Type='MediaClip'] or string-length(normalize-space(.)) > 0">
    ...
    <xsl:template match="/MediaStreamFileResource[Info/MediaStoreFileInfo/Type='MediaClip']/State/StateFlag">
    	<StateFlag><xsl:value-of select="$state_flag"/></StateFlag>
    </xsl:template>
    <xsl:template match="/MediaStreamFileResource[Info/MediaStoreFileInfo/Type='MediaClip']/State/StateMsg">
    	<StateMsg><xsl:value-of select="$state_msg"/></StateMsg>
    </xsl:template>
    <xsl:template match="/MediaStreamFileResource[Info/MediaStoreFileInfo/Type='MediaClip']/Info/UUID">
    	<UUID><xsl:value-of select="$UUID"/></UUID>
    </xsl:template>    
    <xsl:template match="/MediaStreamFileResource[Info/MediaStoreFileInfo/Type='MediaClip']//MediaStoreFileInfo/@MediaStoreFile-Rev">
        <xsl:attribute name="MediaStoreFile-Rev"><xsl:value-of select="$mediaStoreFile_Rev"/></xsl:attribute>
    </xsl:template>
    <xsl:template match="/MediaStreamFileResource[Info/MediaStoreFileInfo/Type='MediaClip']/Info/Properties/Permissions/Write">
    	<Write>false</Write>
    </xsl:template>    
    <!-- 
