...
public class ExternalComponentManager implements ComponentManager {
    ...
    public void addComponent(String subdomain, Component component, Integer port) throws ComponentException {
        ...
        try {
            ...
            externalComponent.connect(host, port, component.getName());
    ...
    public void sendPacket(Component component, Packet packet) {
        ...
        try{
            if(component==null){
            }else if(components.get(component)==null){
                MRLog.error(" sendPacket (components.get(component)==null) " + (components.get(component)==null));
                for (ExternalComponent externalComponent : components.values()) {
                    if(externalComponent!=null){
                        externalComponent.send(packet);
                        return ;
                    }   
                }            
            }else{
            ...
            MRLog.error(" sendPacket no component connected ? " + (component==null));
        }catch(Exception ex){
            getLog().error("", ex)  ;
        }
    }
    public void sendPacket(Component component, String to,  String packet) {
        // Get the ExternalComponent that is wrapping the specified component and ask it to
        // send the packet
        try{
            if(component==null){
                ...
                for (ExternalComponent externalComponent : components.values()) {
                    if(externalComponent!=null){
                        externalComponent.send(to, packet);
                        return ;
                    }   
                }            
            }else{
                components.get(component).send(to, packet);
                return;
            }
            MRLog.error(" sendPacket no component connected ? " + (component==null));
        }catch(Exception ex){
    ...
    private void createDummyLogger() {
        this.logger = new Log() {
            ...
            public void error(String message, Throwable throwable) {
                MRLog.error(message, throwable);
             }
            
            public void error(Throwable throwable) {
                MRLog.error("", throwable);
            }
            ...
            public void warn(String message, Throwable throwable) {
                MRLog.warn(message, throwable);
            }
            
            public void warn(Throwable throwable) {
                MRLog.warn("", throwable);
            }
            ...
            public void info(String message, Throwable throwable) {
                MRLog.debug(message, throwable);
            }
            
            public void info(Throwable throwable) {
                MRLog.debug("", throwable);
            }
