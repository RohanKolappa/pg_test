 ...
 * @since 1.0.0
 */
public class DBTestActionCreateIndex extends DBTestAction {
    private String indexFile = null;
    private Hashtable<String, String> replacelist = new Hashtable<String, String>();
    private Properties props = new Properties();
    
    public DBTestActionCreateIndex() {
        super("DBTestActionCreateCollection");
    }
    
    public DBTestActionCreateIndex(String moduleName) {
        super(moduleName);
    }
    ...
    public void execute(int loop) throws Exception {
        ...
        try {
            ...
            String indexName = props.getProperty("indexName", "") + collectionName;
            db.addIndex(getThreadName(), indexName, docId, collectionName, path, xQueryLocal);
            xQueryLocal = indexName + " " + collectionName + " " + docId + " " + path;
        } catch (Exception ex) {
            MRLog.error(ex);
        } finally {
