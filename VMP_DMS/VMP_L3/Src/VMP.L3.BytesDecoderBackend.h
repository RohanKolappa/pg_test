...
#ifndef _VMP_L3_BytesDecoderBackend_H_
#define _VMP_L3_BytesDecoderBackend_H_
...
BFC_PTR_DECL( VMP_L3_DLL, BytesDecoderBackend )
...
/// \brief Base class of all bytes decoder backends.
///
/// \ingroup VMP_L3

class VMP_L3_DLL BytesDecoderBackend : public BlockDecoderBackend {
...
protected :
	...
	/// \brief Creates a new BytesDecoderBackend.

	BytesDecoderBackend(
		const	BFC::DL::TypeCPtr &	pType
	) :
		BlockDecoderBackend( pType ) {
	}
...
public :
	...
	/// \brief Returns true iff this decoder can decode the encoded data
	...
	/// \brief Decodes the encoded data stored in \a pEncoded.
	///
	/// \returns
	///	The decoded data.
	///
...
#endif // _VMP_L3_BytesDecoderBackend_H_
