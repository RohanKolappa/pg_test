...
BFC_PTR_IMPL_NS( Crypto, AES )

// ============================================================================

const DL::TypeCPtr & Crypto::AES::getClassType() {

	static DL::TypeCPtr i = new DL::Type(
		UUId( "b992a3c6-4703-479c-8cda-a588460c2d43" ),
		"BFC.Crypto.AES",
...
void Crypto::AES::encrypt(
	...
	/*
	* map getByte array block to cipher state
	...
	* map cipher state to getByte array block:
	*/
	s0 =
		(0xFF000000 & Te4[getByte(t0, 3)]) ^
		(0x00FF0000 & Te4[getByte(t1, 2)]) ^
		(0x0000FF00 & Te4[getByte(t2, 1)]) ^
		(0x000000FF & Te4[getByte(t3, 0)]) ^
		rk[0];
	STORE32H(s0, ct);
	s1 =
		(0xFF000000 & Te4[getByte(t1, 3)]) ^
		(0x00FF0000 & Te4[getByte(t2, 2)]) ^
		(0x0000FF00 & Te4[getByte(t3, 1)]) ^
		(0x000000FF & Te4[getByte(t0, 0)]) ^
		rk[1];
	STORE32H(s1, ct+4);
	s2 =
		(0xFF000000 & Te4[getByte(t2, 3)]) ^
		(0x00FF0000 & Te4[getByte(t3, 2)]) ^
		(0x0000FF00 & Te4[getByte(t0, 1)]) ^
		(0x000000FF & Te4[getByte(t1, 0)]) ^
		rk[2];
	STORE32H(s2, ct+8);
	s3 =
		(0xFF000000 & Te4[getByte(t3, 3)]) ^
		(0x00FF0000 & Te4[getByte(t0, 2)]) ^
		(0x0000FF00 & Te4[getByte(t1, 1)]) ^
		(0x000000FF & Te4[getByte(t2, 0)]) ^ 
		rk[3];
	STORE32H(s3, ct+12);

}

void Crypto::AES::decrypt(
...
void Crypto::AES::test() {
	...
	for ( Uint32 i = 0 ; i < ( Uint32 )(sizeof(tests)/sizeof(tests[0])) ; i++ ) {
		setKey( Buffer( tests[i].key, tests[i].keylen ) );
		tmp[ 0 ] = Cipher::encrypt( Buffer( tests[i].pt, 16 ) );
		tmp[ 1 ] = Cipher::decrypt( tmp[ 0 ] );
		if ( tmp[ 0 ] != Buffer( tests[i].ct, 16 )
		  || tmp[ 1 ] != Buffer( tests[i].pt, 16 ) ) { 
			throw InternalError( "Test1" );
		}
		tmp[ 1 ] = tmp[ 0 ] = Buffer::chars( 16, 0x00 );
		for (y = 0; y < 1000; y++) tmp[ 0 ] = Cipher::encrypt( tmp[ 0 ] );
		for (y = 0; y < 1000; y++) tmp[ 0 ] = Cipher::decrypt( tmp[ 0 ] );
		if ( tmp[ 0 ] != tmp[ 1 ] ) {
...
Uint32 Crypto::AES::setup_mix(Uint32 temp)
{
	return (0xFF000000 & Te4[getByte(temp, 2)]) ^
			(0x00FF0000 & Te4[getByte(temp, 1)]) ^
			(0x0000FF00 & Te4[getByte(temp, 0)]) ^
			(0x000000FF & Te4[getByte(temp, 3)]);
}

Uint32 Crypto::AES::setup_mix2(Uint32 temp)
{
	return Td0(255 & Te4[getByte(temp, 3)]) ^
			Td1(255 & Te4[getByte(temp, 2)]) ^
			Td2(255 & Te4[getByte(temp, 1)]) ^
			Td3(255 & Te4[getByte(temp, 0)]);
}
...
const Uint32 Crypto::AES::TE0[256] = {
...
const Uint32 Crypto::AES::Te4[256] = {
...
const Uint32 Crypto::AES::TD0[256] = {
...
const Uint32 Crypto::AES::Td4[256] = {
...
static Crypto::CipherRegisterer< Crypto::AES > aesRegisterer;
