...
#ifndef _BFC_RegEx_KaePattern_H_
#define _BFC_RegEx_KaePattern_H_
...
BFC_PTR_DECL( BFC_RegEx_DLL, KaePattern )
...
/// java.util.regex.KaePattern class. The pattern class represents an immutable
...
/// split apart. The {@link KaeMatcher KaeMatcher} class represents the maching
/// object.
/// 
/// The KaePattern class works primarily off of "compiled" patterns. A typical
/// instantiation of a regular expression looks like:
/// 
/// <pre>
/// KaePattern * p = KaePattern::compile("a*b");
/// KaeMatcher * m = p->createKaeMatcher("aaaaaab");
/// if (m->matches()) ...
/// </pre>
/// 
/// However, if you do not need to use a pattern more than once, it is often times
/// okay to use the KaePattern's static methods insteads. An example looks like this:
/// 
/// <pre>
/// if (KaePattern::matches("a*b", "aaaab")) { ... }
/// </pre>
/// 
/// This class does not currently support unicode. The unicode update for this
/// class is coming soon.
/// 
/// This class is partially immutable. It is completely safe to call createKaeMatcher
/// concurrently in different threads, but the other functions (e.g. split) should
/// not be called concurrently on the same <code>KaePattern</code>.
/// 
...
/// <hr>
/// 
///  <i>Begin Text Extracted And Modified From java.util.regex.KaePattern documentation</i>
/// 
...
/// 
///  <p>The <code>KaePattern</code> engine performs traditional KaeNFA-based matching
...
/// 		by the <CODE>KaeMatcher</CODE> class: Repeated invocations of the
...
/// <P>
/// 
/// <i>End Text Extracted And Modified From java.util.regex.KaePattern documentation</i>
/// 
/// <hr>
///
/// \ingroup BFC_RegEx

class BFC_RegEx_DLL KaePattern : virtual public SObject {

public :

	/// \brief Creates a new KaePattern.
	...
	enum {
		...
		SEARCH_CASE_INSENSITIVE	= 0x01,
		/// \brief Implicitly quoted.
		SEARCH_LITERAL			= 0x02,
		/// \brief '.' is [\\x00-\\x7F].
		SEARCH_DOT_MATCHES_ALL		= 0x04,
		/// \brief '^' and '$' anchor to the beginning and ending of
		///	lines, not all input.
		SEARCH_MULTILINE_MATCHING	= 0x08,
		/// \brief Only '\\n' is line terminator.
		SEARCH_UNIX_LINE_MODE		= 0x10
	};
	...
	fclose(fp);<br>
	<br>
	std::vector&lt;Buffer&gt; lines = KaePattern::split("[\r\n]+", str, true);<br>
	<br>
	</code>
	...
	\param mode	The special mode requested of the <code>KaePattern</code> 											during the split process
	...
	<p>
	<code>
	std::vector&lt;td::string&gt; numbers = KaePattern::findAll("\\d+", string);
	</code>
	<p>

	\param pattern	The pattern for which to search
	\param str			The string to search
	\param mode			The special mode requested of the <code>KaePattern</code>
	...
	<p>
	<code>
	KaePattern::registerKaePattern("ip", "(?:\\d{1,3}\\.){3}\\d{1,3}");<br>
	KaePattern * p1 = KaePattern::compile("{ip}:\\d+");<br>
	KaePattern * p2 = KaePattern::compile("Connection from ({ip}) on port \\d+");<br>
	</code>
	<p>
	Multiple calls to <code>registerKaePattern</code> with the same
	...
	static void registerKaePattern(
		const	Buffer &	name,
		const	Buffer &	pattern,
		const	Uint32		mode = 0
	);
	...
	static void unregisterKaePatterns(
	);
	...
	/// \code
	///	Uint32	pos;
	///	Buffer	match = KaePattern::findNthMatch(
	///		"\\d{1,3}",
	///		"192.168.1.101:22",
	///		1,
	///		pos );
	...
	Buffer getKaePattern(
	) const;

	/// \brief Creates a matcher object using the specified string and
	///	this pattern.

	KaeMatcher * createKaeMatcher(
		const	MatchStringPtr&	str
	);

	// Exceptions...

	BASE_CLASS_GEN_EXCEPTION( "BFC.RegEx.KaePattern" );
...
protected :
	...
	/// then the KaeNFA is modified accordingly.
	///
	/// \param start
	...
	///
	/// If the character was indeed quantified, then the KaeNFA is modified
	/// accordingly.
	///
	/// \param newNode
	...
	KaeNFANode * parseRegisteredKaePattern(
			KaeNFANode **	end
	);
	...
	/**
		...
		\returns The starting node of the KaeNFA constructed from this parse
	 */
...
public :
	...
	KaeMatcher * getKaeMatcher(
	) {
		if ( ! matcher1 ) {
			//matcher1 = new KaeMatcher( this, MatchStringPtr());
			matcher1 = new KaeMatcher( this, 0 );
		}
		return matcher1;
	}
...
private :
	...
	typedef Pair< Buffer, Uint32 >		RegKaePatternPair;
	typedef Map< Buffer, RegKaePatternPair >	RegKaePatternMap;
	...
	static RegKaePatternMap	registeredKaePatterns;

	KaeMatcher *	matcher1;	///< Used when methods like split are called.
	KaeNFANode *	head;		///< The front node of the KaeNFA.
...
class KaeNFACharNode : public KaeNFANode {
...
class KaeNFACICharNode : public KaeNFANode {
...
class KaeNFAStartNode : public KaeNFANode {
...
class KaeNFAEndNode : public KaeNFANode {
...
class KaeNFAQuantifierNode : public KaeNFANode {

public :

	KaeNFAQuantifierNode(
			KaeNFANode *	internal,
		const	Uint32		minMatch = KaePattern::MIN_QMATCH,
		const	Uint32		maxMatch = KaePattern::MAX_QMATCH
	);
...
class KaeNFAGreedyQuantifierNode : public KaeNFAQuantifierNode {

public :

	KaeNFAGreedyQuantifierNode(
			KaeNFANode *	internal,
		const	Uint32		minMatch = KaePattern::MIN_QMATCH,
		const	Uint32		maxMatch = KaePattern::MAX_QMATCH
	) : KaeNFAQuantifierNode( internal, minMatch, maxMatch ) {
	}
...
class KaeNFALazyQuantifierNode : public KaeNFAQuantifierNode {

public :

	KaeNFALazyQuantifierNode(
			KaeNFANode *	internal,
		const	Uint32		minMatch = KaePattern::MIN_QMATCH,
		const	Uint32		maxMatch = KaePattern::MAX_QMATCH
	) : KaeNFAQuantifierNode( internal, minMatch, maxMatch ) {
...
class KaeNFAPossessiveQuantifierNode : public KaeNFAQuantifierNode {

public :

	KaeNFAPossessiveQuantifierNode(
			KaeNFANode *	internal,
		const	Uint32		minMatch = KaePattern::MIN_QMATCH,
		const	Uint32		maxMatch = KaePattern::MAX_QMATCH
	) : KaeNFAQuantifierNode( internal, minMatch, maxMatch ) {
...
class KaeNFAAcceptNode : public KaeNFANode {
...
class KaeNFAClassNode : public KaeNFANode {
...
class KaeNFACIClassNode : public KaeNFANode {
...
class KaeNFASubStartNode : public KaeNFANode {
...
class KaeNFAOrNode : public KaeNFANode {
...
class KaeNFAQuoteNode : public KaeNFANode {
...
class KaeNFACIQuoteNode : public KaeNFANode {
...
class KaeNFALookAheadNode : public KaeNFANode {
...
class KaeNFALookBehindNode : public KaeNFANode {

public :

	KaeNFALookBehindNode(
		const	Buffer &	str,
		const	Bool		positive
...
class KaeNFAStartOfLineNode : public KaeNFANode {

public :

	KaeNFAStartOfLineNode(
	) {
	}
...
class KaeNFAEndOfLineNode : public KaeNFANode {
...
class KaeNFAReferenceNode : public KaeNFANode {
...
class KaeNFAStartOfInputNode : public KaeNFANode {

public :

	KaeNFAStartOfInputNode(
	) {
		startOfInput = true;
	}
...
class KaeNFAEndOfInputNode : public KaeNFANode {

public :

	KaeNFAEndOfInputNode(
...
class KaeNFAWordBoundaryNode : public KaeNFANode {

public :

	KaeNFAWordBoundaryNode(
		const	Bool		positive
	) : pos( positive ) {
	}
...
class KaeNFAEndOfMatchNode : public KaeNFANode {

public :

	KaeNFAEndOfMatchNode(
	) {
	}
...
class KaeNFAGroupHeadNode : public KaeNFANode {
...
class KaeNFAGroupTailNode : public KaeNFANode {
...
class KaeNFAGroupLoopPrologueNode : public KaeNFANode {

public :

	KaeNFAGroupLoopPrologueNode(
		const	int		groupIndex
	) : gi( groupIndex ) {
	}
...
class KaeNFAGroupLoopNode : public KaeNFANode {
...
#endif // _BFC_RegEx_KaePattern_H_
