...
QtUIParser::QtUIParser() {

	doDebug = true;

	Buffer	customName = Buffer( ::getenv( "HOME" ) ) + "/.VMP/customwidgets.list";

	File	customFile;

	if ( customFile.open( customName, File::Read ) ) {
		Buffer customData;
		customData = customFile.dumpToBuffer();
		BufferArray entries = BufferParser::tokenize( customData, "\n\r\t " );
		for ( int i = 0 ; i < entries.size() ; i++ ) {
			customList += BufQString( entries[ i ] );
		}
	}
...
void QtUIParser::setDebug( const bool b ) {

	doDebug = b;

}

void QtUIParser::msgWrn( const Buffer& b ) {
...
QDomElement QtUIParser::parseContainer(
		QDomDocument	oDoc,
		QDomElement	iElt,
...
//	msgDbg( "parseBox(): parsing subelements..." );
	...
	for ( QDomElement n = iElt.firstChild().toElement()
		...
		else if ( customList.contains( className ) ) {
			subElt = parseSimpleWidget(
				oDoc, n,
...
QDomElement QtUIParser::parseSimpleWidget(
		QDomDocument		oDoc,
		QDomElement		iElt,
...
QDomElement QtUIParser::parseComboBox(
		QDomDocument		oDoc,
		QDomElement		iElt ) {
...
QDomElement QtUIParser::parseButtonGroup(
		QDomDocument		oDoc,
		QDomElement		iElt ) {
...
QDomElement QtUIParser::parseWidgetStack(
		QDomDocument		oDoc,
		QDomElement		iElt ) {

	QDomElement	oElt;		// StackedFrame
	QDomElement	n;

	oElt = oDoc.createElement( "StackedFrame" );
	...
	for ( n = iElt.firstChild().toElement()
		...
		if ( tagName == "property" ) {
			QString a = n.attribute( "name" );
			if ( a == "name" ) {
				QDomAttr	oNameAttr;	// StackedFrame name
				oNameAttr = oDoc.createAttribute( "name" );
				oNameAttr.setValue( n.text() );
				oElt.setAttributeNode( oNameAttr );
			}
		...
		oElt.appendChild(
			parseContainer( oDoc, n, "StackedItem", "" ) );
