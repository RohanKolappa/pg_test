...
BFC_PTR_IMPL_NS( Crypto, Anubis )

// ============================================================================

const DL::TypeCPtr & Crypto::Anubis::getClassType() {

	static DL::TypeCPtr i = new DL::Type(
		UUId( "2614617d-306b-4fd0-9198-d0f63249e61a" ),
		"BFC.Crypto.Anubis",
...
Uint32 Crypto::Anubis::getKeySize(
...
void Crypto::Anubis::setKey(
	 ...
	 */
	for ( Uint32 i = 0, pos = 0; i < N; i++, pos += 4) {
		kappa[i] =
			(key[pos	 ] << 24) ^
			(key[pos + 1] << 16) ^
			(key[pos + 2] <<  8) ^
...
void Crypto::Anubis::encrypt(
	const	Buffer &	pPlainText,
		Buffer &	pCipherText ) {

	process( pPlainText, pCipherText, roundKeyEnc );

}

void Crypto::Anubis::decrypt(
	const	Buffer &	pCipherText,
		Buffer &	pPlainText ) {

	process( pCipherText, pPlainText, roundKeyDec );

}

// ============================================================================

void Crypto::Anubis::process(
	const	Buffer &	pPlainText,
		Buffer &	pCipherText,
	 ...
	 */
	 for ( Uint32 i = 0, pos = 0; i < 4; i++, pos += 4) {
		state[i] =
			(pt[pos	 ] << 24) ^
			(pt[pos + 1] << 16) ^
			(pt[pos + 2] <<  8) ^
...
void Crypto::Anubis::test() {
	...
	for ( Uint32 x = 0; x < (sizeof(tests)/sizeof(tests[0])); x++) {
		...
		if ( nct != ct
		  || npt != pt ) {
			throw InternalError( "Anubis failed (pass 1)!" );
		}
		...
		if ( tmp != ct ) {
			throw InternalError( "Anubis failed (pass 2)!" );
		}
...
const Uint32 Crypto::Anubis::T0[256] = {
...
const Uint32 Crypto::Anubis::T1[256] = {
...
const Uint32 Crypto::Anubis::T2[256] = {
...
const Uint32 Crypto::Anubis::T3[256] = {
...
const Uint32 Crypto::Anubis::T4[256] = {
...
const Uint32 Crypto::Anubis::T5[256] = {
...
static Crypto::CipherRegisterer< Crypto::Anubis > anubisRegisterer;
