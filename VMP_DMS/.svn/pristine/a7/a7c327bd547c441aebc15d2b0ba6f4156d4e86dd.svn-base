// ============================================================================
// 
// Copyright (c) 2003-2015 Barco N.V.
// 
// ============================================================================
// 
// Filename:
...
Buffer DOM::encodeAttr(
	...
	for ( Uint32 i = 0 ; i < len ; i++ ) {
		...
		else if ( encodeQuotes && c == '"' ) {
			tmp += "&quot;";
		}
		else if ( c == '&' ) {
			tmp += "&amp;";
		}
		else if ( c == '>' && i >= 2 && str[ i-1 ] == ']' && str[ i-2 ] == ']' ) {
			tmp += "&gt;";
		}
		else {
			tmp += c;
...
Buffer DOM::quotedValue(
	const	Buffer &	data ) {

	Uchar quote = data.findFwd( '\'' ) == data.getLength()
					? '\''
					: '"';
	return Buffer( quote ) + data + Buffer( quote );

}

// ============================================================================

void DOM::splitNamespace(
		Buffer &	prefix,
		Buffer &	name,
	const	Buffer &	qName,
	const	Bool		/* hasURI */ ) {

	Uint32 i = qName.findFwd( ':', 0 );

	if ( i == qName.getLength() ) {
		prefix.kill();
		name = qName;
	}
	else {
...
static const RangePtr g_base_end = g_base_begin + sizeof(g_base_begin)/sizeof(Range);

inline static Bool isBaseChar(const Uchar &c)
{
...
static const RangePtr g_ideographic_end = g_ideographic_begin + sizeof(g_ideographic_begin)/sizeof(Range);

inline static Bool isIdeographic(const Uchar &c)
{
...
static const RangePtr g_combining_end = g_combining_begin + sizeof(g_combining_begin)/sizeof(Range);

inline static Bool isCombiningChar(const Uchar &c)
{
...
static const RangePtr g_digit_end = g_digit_begin + sizeof(g_digit_begin)/sizeof(Range);

inline static Bool isDigit(const Uchar &c)
{
...
static const RangePtr g_extender_end = g_extender_begin + sizeof(g_extender_begin)/sizeof(Range);

inline static Bool isExtender(const Uchar &c)
{
...
inline static Bool isLetter(const Uchar &c)
{
...
static Bool isChar(const Uchar &c)
{
	return c == 0x0009
			|| c == 0x000A
			|| c == 0x000D
...
static Bool isNameChar(const Uchar &c)
{
	return isLetter(c)
			|| isDigit(c)
			|| c == '.'
...
static Bool isPubidChar(const Uchar &c)
{
	if ( ( c >= 'a' && c <= 'z' )
		|| ( c >= 'A' && c <= 'Z' )
		|| ( c >= '0' && c <= '9' ) )
...
Buffer DOM::fixedXmlName(const Buffer &_name, Bool *ok, Bool namespaces)
{
	Buffer name, prefix;
	if (namespaces)
		splitNamespace(prefix, name, _name, true);
	else
		name = _name;

	if (name.isEmpty()) {
		if ( ok ) {*ok = false; throw InvalidArgument();}
		return Buffer();
	}

//	if (DOM::ImplementationImpl::invalidDataPolicy == DOM::AcceptInvalidChars) {
//		if ( ok ) *ok = true;
//		return _name;
//	}
	...
	Buffer result;
	Bool firstChar = true;
	for (Uint32 i = 0; i < name.getLength(); ++i) {
		Uchar c = name[ i ];
		if (firstChar) {
			if (isLetter(c) || c == '_' || c == ':') {
				result+= c;
				firstChar = false;
			} else /* if (DOM::ImplementationImpl::invalidDataPolicy == DOM::ReturnNullNode) */ {
				if ( ok ) { *ok = false; throw InvalidArgument(); }
				return Buffer();
			}
		} else {
...
Buffer DOM::fixedCharData(const Buffer &data, Bool *ok)
{
//	if (DOM::ImplementationImpl::invalidDataPolicy == DOM::AcceptInvalidChars) {
//		*ok = true;
//		return data;
//	}
	...
	for (Uint32 i = 0; i < data.getLength(); ++i) {
		...
		if (isChar(c)) {
			...
			*ok = false; throw InvalidArgument();
			return Buffer();
		}
	}
...
Buffer DOM::fixedComment(const Buffer &data, Bool *ok)
{
//	if (DOM::ImplementationImpl::invalidDataPolicy == DOM::AcceptInvalidChars) {
//		*ok = true;
//		return data;
//	}
	...
	for (;;) {
		Uint32 idx = fixedData.findFwd( "--" );
		if (idx == fixedData.getLength())
			break;
//		if (DOM::ImplementationImpl::invalidDataPolicy == DOM::ReturnNullNode) {
			*ok = false; throw InvalidArgument();
			return Buffer();
//		}
...
Buffer DOM::fixedCDataSection(const Buffer &data, Bool *ok)
{
//	if (DOM::ImplementationImpl::invalidDataPolicy == DOM::AcceptInvalidChars) {
//		*ok = true;
//		return data;
...
Buffer DOM::fixedPIData(const Buffer &data, Bool *ok)
{
//	if (DOM::ImplementationImpl::invalidDataPolicy == DOM::AcceptInvalidChars) {
//		*ok = true;
//		return data;
...
Buffer DOM::fixedPubidLiteral(const Buffer &data, Bool *ok)
{
//	if (DOM::ImplementationImpl::invalidDataPolicy == DOM::AcceptInvalidChars) {
//		*ok = true;
//		return data;
//	}
	...
	if ( result.findFwd( '\'' ) != result.getLength()
	  && result.findFwd( '"' ) != result.getLength() ) {
//		if (DOM::ImplementationImpl::invalidDataPolicy == DOM::ReturnNullNode) {
			*ok = false; throw InvalidArgument();
			return Buffer();
//		} else {
...
Buffer DOM::fixedSystemLiteral(
	const	Buffer &	data,
		Bool *		ok ) {

//	if ( ImplementationImpl::invalidDataPolicy == AcceptInvalidChars) {
//		*ok = true;
//		return data;
//	}
