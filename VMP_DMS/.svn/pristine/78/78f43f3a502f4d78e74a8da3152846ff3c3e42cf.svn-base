...
BFC_PTR_IMPL_NS( Crypto, SHA512 )

// ============================================================================

const DL::TypeCPtr & Crypto::SHA512::getClassType() {

	static DL::TypeCPtr i = new DL::Type(
		UUId( "6f7d3cc2-a758-4317-b80f-828620188bde" ),
		"BFC.Crypto.SHA512",
...
void Crypto::SHA512::process(
	const	Buffer &	pData ) {

	msgBuf += pData;
	msgLen += pData.getLength();

	while ( msgBuf.getLength() >= 128 ) {
		compress();
		msgBuf = msgBuf( 128 );
	}
...
void Crypto::SHA512::done(
		Buffer &	pOutput ) {

	msgLen <<= 3;
	msgBuf += ( Uchar )0x80;

	if ( msgBuf.getLength() > 112 ) {
		msgBuf += Buffer::chars( 256 - msgBuf.getLength(), 0x00 );
	}
	else {
	...
	Uchar * ptr = pOutput.getVarPtr();

	for ( Uint32 i = 0 ; i < hashSize ; i += 8 ) {
		STORE64H( state[ i / 8 ], ptr + i );
	}
...
void Crypto::SHA512::compress() {
	...
	for ( Uint32 i = 0 ; i < 80 ; i++ ) {
		Uint64 t0 = S[7] + Sigma1(S[4]) + Ch(S[4], S[5], S[6]) + K[i] + W[i];
		Uint64 t1 = Sigma0(S[0]) + Maj(S[0], S[1], S[2]);
		S[7] = S[6];
		S[6] = S[5];
		S[5] = S[4];
		S[4] = S[3] + t0;
...
const Uint64 Crypto::SHA512::K[80] = {
...
static Crypto::HashRegisterer< Crypto::SHA512 > sha512Registerer;
