...
void FSNode::rename(
	...
	FSPath	oldPath = getPath();
	FSPath	newPath = oldPath;

	newPath.replaceLastComponent( pNewName );

	Buffer	oldName = oldPath.toBuffer();
	Buffer	newName = newPath.toBuffer();

	if ( ::rename( oldName.getCStringPtr(), newName.getCStringPtr() ) ) {
...
Uint64 FSNode::getTotalSize() const {

	return ( data->present
		? ( Uint64 )data->stats.st_size
		: ( Uint64 )0 );
