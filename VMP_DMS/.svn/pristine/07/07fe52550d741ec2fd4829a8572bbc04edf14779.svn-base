...
#ifndef _BFC_Base_BufferParser_H_
#define _BFC_Base_BufferParser_H_
...
class BFC_Light_DLL BufferParser {
...
public :
	...
	static BufferArray tokenize(
		...
		return tokenize( source, Buffer( separators ), skipEmpty );
	}

	static Buffer merge(
		const	BufferArray &	pTokens,
		const	Buffer &	pSeparator = Buffer()
	);
