...
BFC_PTR_IMPL_NS( RegEx, NewMatcher )

// ============================================================================

RegEx::NewMatcher::NewMatcher() {
...
void RegEx::NewMatcher::clearGroups() {
...
Buffer RegEx::NewMatcher::replaceWithGroups(const Buffer & str) {
...
Uint32 RegEx::NewMatcher::getFlags() const
{
	return flags;
}

Buffer RegEx::NewMatcher::getText() const
{
	return str;
}

Bool RegEx::NewMatcher::matches() {
...
Bool RegEx::NewMatcher::findFirstMatch()
{
	starts[0] = 0;
	flags = 0;
	clearGroups();
...
Bool RegEx::NewMatcher::findNextMatch()
{
...
BufferArray RegEx::NewMatcher::findAll()
{
	BufferArray ret;
	reset();
	while (findNextMatch())
...
int RegEx::NewMatcher::getStartingIndex(const int groupNum) const
{
	if (groupNum < 0 || groupNum >= gc) return -1;
	return starts[groupNum];
}

int RegEx::NewMatcher::getEndingIndex(const int groupNum) const
{
	if (groupNum < 0 || groupNum >= gc) return -1;
	return ends[groupNum];
}

Buffer RegEx::NewMatcher::getGroup(const int groupNum) const
{
...
BufferArray RegEx::NewMatcher::getGroups(const Bool includeGroupZero) const
{
