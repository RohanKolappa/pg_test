...
// ============================================================================
//
// This file is part of "BFC::NTLM".
//
// "BFC::NTLM" is free software; you can redistribute it and/or modify it
// under the terms of the GNU Lesser General Public License as published
// by the Free Software Foundation; either version 2.1 of the License, or
// (at your option) any later version.
//
// "BFC::NTLM" is distributed in the hope that it will be useful, but
// WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
// Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with "BFC::NTLM"; if not, write to:
...
#ifndef _BFC_NTLM_DLL_H_
#define _BFC_NTLM_DLL_H_
...
/// \defgroup BFC_NTLM BFC::NTLM (??? module)
///
...
#if ! defined( BFC_NTLM_DLL )
#  if defined( BFC_NTLM_EXPORTS )
#    define BFC_NTLM_DLL JBS_DLL_EXPORT
#  else
#    define BFC_NTLM_DLL JBS_DLL_IMPORT
#  endif
#endif
...
/// Resources section at the end of this document): Paul Ashton's work on the
/// NTLM security holes, the encryption documentation from Samba, and network
/// snooping. Since most of this info is reverse-engineered it is bound to
/// contain errors; however, at least one client and one server have been
/// implemented according to this data and work successfully in conjunction
/// with M$'s browsers, proxies and servers.
/// 
/// Note that this scheme is not as secure as Digest and some other schemes;
/// it is slightly better than the Basic authentication scheme, however.
/// 
/// Also note that this scheme is not an http authentication scheme - it's a
/// connection authentication scheme which happens to (mis-)use http status
/// codes and headers (and even those incorrectly).
///
/// \section dummy0 NTLM Handshake
/// 
...
/// byte is an 8-bit field; short is a 16-bit field. All fields are unsigned.
/// Numbers are stored in little-endian order. Struct fields named zero
/// contain all zeroes. An array length of "*" indicates a variable length
/// field. Hexadecimal numbers and quoted characters in the comments of the
/// struct indicate fixed values for the given field.
/// 
/// The field flags is presumed to contain flags, but their significance is
/// unknown; the values given are just those found in the packet traces.
///
/// \subsection dummy1 Type-1 Message
/// 
...
/// offsets refer to the offset of the specific field within the message, and
/// the lengths are the length of specified field. For example, in the above
/// message host_off = 32 and dom_off = host_off + host_len. Note that the
/// lengths are included twice (for some unfathomable reason).
///
/// \subsection dummy2 Type-2 Message
/// 
...
/// length field contains the length of the complete message, which in this
/// case is always 40.
///
/// \subsection dummy3 Type-3 Message
/// 
...
/// \endverbatim
/// 
/// The host, domain, and username strings are in Unicode (UTF-16,
/// little-endian) and are not nul-terminated; the host and domain names are
/// in upper case. The lengths of the response strings are 24.
///
/// \subsection dummy4 Password Hashes
/// 
/// To calculate the two response strings two password hashes are used: the
/// LanManager password hash and the NT password hash. These are described in
/// detail at the beginning of the Samba ENCRYPTION.html document. However, a
/// few things are not clear (such as what the magic constant for the
/// LanManager hash is), so here is some almost-C code which calculates the
/// two responses. Inputs are passw and nonce, the results are in lm_resp and
/// nt_resp.
/// 
/// \code
...
/// \endcode
/// 
/// \section dummy5 Keeping the connection alive
/// 
/// As mentioned above, this scheme authenticates connections, not requests.
/// This manifests itself in that the network connection must be kept alive
/// during the second part of the handshake, i.e. between the receiving of the
/// type-2 message from the server (step 4) and the sending of the type-3
/// message (step 5). Each time the connection is closed this second part
/// (steps 3 through 6) must be repeated over the new connection (i.e. it's
/// not enough to just keep sending the last type-3 message). Also, once the
/// connection is authenticated, the Authorization header need not be sent
/// anymore while the connection stays open, no matter what resource is
/// accessed.
/// 
/// For implementations wishing to work with M$'s software this means that
/// they must make sure they use either HTTP/1.0 keep-alive's or HTTP/1.1
/// persistent connections, and that they must be prepared to do the second
/// part of the handshake each time the connection was closed and is reopened.
/// Server implementations must also make sure that HTTP/1.0 responses contain
/// a Content-length header (as otherwise the connection must be closed after
/// the response), and that HTTP/1.1 responses either contain a Content-length
/// header or use the chunked transfer encoding.
///
/// \section Example
/// 
/// Here is an actual example of all the messages. Assume the host name is
