...
//	msgDbg( "FSDir::refresh(\"" + dirName + "\")" );
	...
	if ( ( hndl = FindFirstFileA( p, &data ) ) == INVALID_HANDLE_VALUE ) {
	...
	DIR *		dir;
	Buffer		dirName = path.toBuffer();
	const char *	dirPtr = dirName.getCStringPtr();
...
// FIXME: why not immediatly use a SortArray< Buffer > instead ?
