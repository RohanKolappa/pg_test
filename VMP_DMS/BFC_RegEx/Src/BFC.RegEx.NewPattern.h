...
#ifndef _BFC_RegEx_NewPattern_H_
#define _BFC_RegEx_NewPattern_H_
...
BFC_PTR_DECL( BFC_RegEx_DLL, NewPattern )
...
/// java.util.regex.NewPattern class. The pattern class represents an immutable
...
/// split apart. The {@link NewMatcher NewMatcher} class represents the maching
/// object.
/// 
/// The NewPattern class works primarily off of "compiled" patterns. A typical
/// instantiation of a regular expression looks like:
/// 
/// <pre>
/// NewPattern * p = NewPattern::compile("a*b");
/// NewMatcher * m = p->createNewMatcher("aaaaaab");
/// if (m->matches()) ...
/// </pre>
/// 
/// However, if you do not need to use a pattern more than once, it is often times
/// okay to use the NewPattern's static methods insteads. An example looks like this:
/// 
/// <pre>
/// if (NewPattern::matches("a*b", "aaaab")) { ... }
/// </pre>
/// 
/// This class does not currently support unicode. The unicode update for this
/// class is coming soon.
/// 
/// This class is partially immutable. It is completely safe to call createNewMatcher
/// concurrently in different threads, but the other functions (e.g. split) should
/// not be called concurrently on the same <code>NewPattern</code>.
/// 
...
/// <hr>
/// 
///  <i>Begin Text Extracted And Modified From java.util.regex.NewPattern documentation</i>
/// 
...
/// 
///  <p>The <code>NewPattern</code> engine performs traditional NFA-based matching
...
/// 		by the <CODE>NewMatcher</CODE> class: Repeated invocations of the
...
/// <P>
/// 
/// <i>End Text Extracted And Modified From java.util.regex.NewPattern documentation</i>
/// 
/// <hr>
///
/// \ingroup BFC_RegEx

class BFC_RegEx_DLL NewPattern : virtual public SObject {

public :

	/// \brief Creates a new NewPattern.
	...
	fclose(fp);<br>
	<br>
	std::vector&lt;Buffer&gt; lines = NewPattern::split("[\r\n]+", str, true);<br>
	<br>
	</code>
	...
	\param mode	The special mode requested of the <code>NewPattern</code> 											during the split process
	...
	<p>
	<code>
	std::vector&lt;td::string&gt; numbers = NewPattern::findAll("\\d+", string);
	</code>
	<p>

	\param pattern	The pattern for which to search
	\param str			The string to search
	\param mode			The special mode requested of the <code>NewPattern</code>
	...
	<p>
	<code>
	NewPattern::registerNewPattern("ip", "(?:\\d{1,3}\\.){3}\\d{1,3}");<br>
	NewPattern * p1 = NewPattern::compile("{ip}:\\d+");<br>
	NewPattern * p2 = NewPattern::compile("Connection from ({ip}) on port \\d+");<br>
	</code>
	<p>
	Multiple calls to <code>registerNewPattern</code> with the same
	...
	static void registerNewPattern(
		const	Buffer &	name,
		const	Buffer &	pattern,
		const	Uint32		mode = 0
	);
	...
	static void unregisterNewPatterns(
	);
	...
	/// \code
	///	Uint32	pos;
	///	Buffer	match = NewPattern::findNthMatch(
	///		"\\d{1,3}",
	///		"192.168.1.101:22",
	///		1,
	///		pos );
	...
	Buffer getNewPattern(
	) const;

	// Exceptions...

	BASE_CLASS_GEN_EXCEPTION( "BFC.RegEx.NewPattern" );
...
protected :
	...
	NewNFANode * parseRegisteredNewPattern(
			NewNFANode **	end
	);
...
public :
	...
	NewMatcher * getNewMatcher(
	) {
		if ( ! matcher ) {
			matcher = new NewMatcher( this );
		}
		return matcher;
	}
...
private :
	...
	typedef Pair< Buffer, Uint32 >		RegNewPatternPair;
	typedef Map< Buffer, RegNewPatternPair >	RegNewPatternMap;
	...
	static RegNewPatternMap	registeredNewPatterns;

	NewMatcher *	matcher;	///< Used when methods like split are called.
	NewNFANode *	head;		///< The front node of the NFA.
...
class NewNFACharNode : public NewNFANode {
...
class NewNFACICharNode : public NewNFANode {
...
class NewNFAStartNode : public NewNFANode {
...
class NewNFAEndNode : public NewNFANode {
...
class NewNFAQuantifierNode : public NewNFANode {

public :

	NewNFAQuantifierNode(
			NewNFANode *	internal,
		const	Uint32		minMatch = NewPattern::MIN_QMATCH,
		const	Uint32		maxMatch = NewPattern::MAX_QMATCH
	);
...
class NewNFAGreedyQuantifierNode : public NewNFAQuantifierNode {

public :

	NewNFAGreedyQuantifierNode(
			NewNFANode *	internal,
		const	Uint32		minMatch = NewPattern::MIN_QMATCH,
		const	Uint32		maxMatch = NewPattern::MAX_QMATCH
	) : NewNFAQuantifierNode(
		internal, minMatch, maxMatch ) {
	}
...
class NewNFALazyQuantifierNode : public NewNFAQuantifierNode {

public :

	NewNFALazyQuantifierNode(
			NewNFANode *	internal,
		const	Uint32		minMatch = NewPattern::MIN_QMATCH,
		const	Uint32		maxMatch = NewPattern::MAX_QMATCH
	) : NewNFAQuantifierNode(
...
class NewNFAPossessiveQuantifierNode : public NewNFAQuantifierNode {

public :

	NewNFAPossessiveQuantifierNode(
			NewNFANode *	internal,
		const	Uint32		minMatch = NewPattern::MIN_QMATCH,
		const	Uint32		maxMatch = NewPattern::MAX_QMATCH
	) : NewNFAQuantifierNode(
...
class NewNFAAcceptNode : public NewNFANode {
...
class NewNFADotNode : public NewNFANode {
...
class NewNFAClassNode : public NewNFANode {
...
class NewNFACIClassNode : public NewNFANode {
...
class NewNFASubStartNode : public NewNFANode {
...
class NewNFAOrNode : public NewNFANode {
...
class NewNFAQuoteNode : public NewNFANode {
...
class NewNFACIQuoteNode : public NewNFANode {
...
class NewNFALookAheadNode : public NewNFANode {
...
class NewNFALookBehindNode : public NewNFANode {

public :

	NewNFALookBehindNode(
		const	Buffer &	str,
		const	Bool		positive
	) :
...
class NewNFAStartOfLineNode : public NewNFANode {

public :

//	NewNFAStartOfLineNode(
//	) {
//	}
...
class NewNFAEndOfLineNode : public NewNFANode {
...
class NewNFAReferenceNode : public NewNFANode {
...
class NewNFAStartOfInputNode : public NewNFANode {

public :

	NewNFAStartOfInputNode(
	) {
		startOfInput = true;
	}
...
class NewNFAEndOfInputNode : public NewNFANode {

public :

	NewNFAEndOfInputNode(
...
class NewNFAWordBoundaryNode : public NewNFANode {

public :

	NewNFAWordBoundaryNode(
		const	Bool		positive
	) :
		pos( positive ) {
	}
...
class NewNFAEndOfMatchNode : public NewNFANode {

public :

//	NewNFAEndOfMatchNode(
//	) {
//	}
...
class NewNFAGroupHeadNode : public NewNFANode {
...
class NewNFAGroupTailNode : public NewNFANode {
...
class NewNFAGroupLoopPrologueNode : public NewNFANode {

public :

	NewNFAGroupLoopPrologueNode(
		const	int		groupIndex
	) :
		gi( groupIndex ) {
	}
...
class NewNFAGroupLoopNode : public NewNFANode {
...
#endif // _BFC_RegEx_NewPattern_H_
