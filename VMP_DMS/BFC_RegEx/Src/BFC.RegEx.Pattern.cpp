...
BFC_PTR_IMPL_NS( RegEx, Pattern )

// ============================================================================

RegEx::Pattern::RegPatternMap RegEx::Pattern::registeredPatterns;
...
void RegEx::Pattern::compile(
	...
	if ( flags & LITERAL ) {
		head = new NFAStartNode;
		if ( flags & CASE_INSENSITIVE ) {
			head->next = new NFACIQuoteNode( pattern );
		}
		else {
			head->next = new NFAQuoteNode( pattern );
		}
		head->next->next = new NFAEndNode;
	}
	else {
		head = parse( 0, 0, &end );
		if ( ! head ) {
		...
		if ( ! head->isStartOfInputNode() ) {
			NFANode * n = new NFAStartNode;
			n->next = head;
			head = n;
		}
		end->next = new NFAEndNode;
	}
...
Buffer RegEx::Pattern::replace(
	...
	getMatcher()->setString( originText );

	while ( getMatcher()->findNextMatch() ) {
		ret += originText( li, getMatcher()->getStartingIndex() - li );
		ret += getMatcher()->replaceWithGroups( replaceText );
		li = getMatcher()->getEndingIndex();
	}
...
Buffer RegEx::Pattern::classUnion(
	const	Buffer &	s1,
...
Buffer RegEx::Pattern::classIntersect(
	const	Buffer &	s1,
	const	Buffer &	s2 ) const {
...
Buffer RegEx::Pattern::classNegate(
...
Buffer RegEx::Pattern::classCreateRange(
		Uchar		low,
		Uchar		hi ) const {
...
int RegEx::Pattern::getInt(int start, int end)
{
	int ret = 0;
	for (; start <= end; ++start) ret = ret * 10 + (pattern[start] - '0');
	return ret;
}
Bool RegEx::Pattern::quantifyCurly(int & sNum, int & eNum)
{
	Bool	good = 1;
...
RegEx::NFANode * RegEx::Pattern::quantifyGroup(
	RegEx::NFANode * start, RegEx::NFANode * stop, const int gn)
{
	NFANode * newNode = 0;
	int type = 0;
	...
	if (curInd < pattern.getLength())
	{
		...
		case '*':
			...
			newNode = new NFAGroupLoopPrologueNode(gn);
			newNode->next = new NFAGroupLoopNode(start, MIN_QMATCH, MAX_QMATCH, gn, type);
		...
		case '?':
			...
			newNode = new NFAGroupLoopPrologueNode(gn);
			newNode->next = new NFAGroupLoopNode(start, MIN_QMATCH, 1, gn, type);
			stop->next = newNode->next;
			return newNode;
		case '+':
			++curInd;
			...
			newNode = new NFAGroupLoopPrologueNode(gn);
			newNode->next = new NFAGroupLoopNode(start, 1, MAX_QMATCH, gn, type);
			stop->next = newNode->next;
			return newNode;
		case '{':
			{
				...
				if (quantifyCurly(s, e))
				{
					...
					newNode = new NFAGroupLoopPrologueNode(gn);
					newNode->next = new NFAGroupLoopNode(start, s, e, gn, type);
					stop->next = newNode->next;
					return newNode;
				}
			}
...
RegEx::NFANode * RegEx::Pattern::quantify(RegEx::NFANode * newNode)
{
	if (curInd < pattern.getLength())
	{
		char ch = ( char )( (curInd + 1 >= pattern.getLength()) ? -1 : pattern[curInd + 1] );
		...
		case '*':
			...
			switch (ch)
			{
			case '?': ++curInd; newNode = new NFALazyQuantifierNode( newNode, MIN_QMATCH, MAX_QMATCH); break;
			case '+': ++curInd; newNode = new NFAPossessiveQuantifierNode( newNode, MIN_QMATCH, MAX_QMATCH); break;
			default:						newNode = new NFAGreedyQuantifierNode( newNode, MIN_QMATCH, MAX_QMATCH); break;
			}
			break;
		case '?':
			++curInd;
			switch (ch)
			{
			case '?': ++curInd; newNode = new NFALazyQuantifierNode( newNode, MIN_QMATCH, 1); break;
			case '+': ++curInd; newNode = new NFAPossessiveQuantifierNode( newNode, MIN_QMATCH, 1); break;
			default:						newNode = new NFAGreedyQuantifierNode( newNode, MIN_QMATCH, 1); break;
			}
			break;
		case '+':
			++curInd;
			switch (ch)
			{
			case '?': ++curInd; newNode = new NFALazyQuantifierNode( newNode, 1, MAX_QMATCH); break;
			case '+': ++curInd; newNode = new NFAPossessiveQuantifierNode( newNode, 1, MAX_QMATCH); break;
			default:						newNode = new NFAGreedyQuantifierNode( newNode, 1, MAX_QMATCH); break;
			}
			break;
		case '{':
			{
				...
				if (quantifyCurly(s, e))
				{
					...
					switch (ch)
					{
					case '?': ++curInd; newNode = new NFALazyQuantifierNode( newNode, s, e); break;
					case '+': ++curInd; newNode = new NFAPossessiveQuantifierNode( newNode, s, e); break;
					default:						newNode = new NFAGreedyQuantifierNode( newNode, s, e); break;
					}
				}
			}
			break;
...
Buffer RegEx::Pattern::parseClass()
{
	Buffer t, ret = "";
	char ch, c1, c2;
...
Buffer RegEx::Pattern::parsePosix()
{
...
RegEx::NFANode * RegEx::Pattern::parseBackref()
{
	...
	{
		raiseError();
		return new NFAReferenceNode(-1);
	}
	curInd = ci;
	return new NFAReferenceNode(ref);

	#undef is_dig
	#undef to_int
}
Buffer RegEx::Pattern::parseOctal()
{
...
Buffer RegEx::Pattern::parseEscape(Bool & inv, Bool & quo)
{
...
RegEx::NFANode * RegEx::Pattern::parseRegisteredPattern(
		NFANode **	end ) {
	...
	if ( ! registeredPatterns.contains( s ) ) {
	...
	pattern = registeredPatterns[ s ].left();
	flags = registeredPatterns[ s ].right();
	curInd = 0;
...
RegEx::NFANode * RegEx::Pattern::parseBehind(const Bool pos, RegEx::NFANode ** end)
{
	Buffer t = "";
	while (curInd < pattern.getLength() && pattern[curInd] != ')')
	{
	...
	return *end = new NFALookBehindNode(t, pos);
}
RegEx::NFANode * RegEx::Pattern::parseQuote()
{
	Bool done = 0;
	Buffer s = "";
	...
	if ((flags & RegEx::Pattern::CASE_INSENSITIVE) != 0) return new NFACIQuoteNode(s);
	return new NFAQuoteNode(s);
}

RegEx::NFANode * RegEx::Pattern::parse(
	const	Bool			inParen,
	const	Bool			inOr,
		RegEx::NFANode **	end ) {
	...
	if ( inParen ) {
		if ( pattern[ curInd ] == '?' ) {
			...
			if ( pattern[curInd] == ':' ) {
				...
				while ( ! done ) {
					if ( curInd >= pattern.getLength() ) {
						...
						case ')':
							++curInd;
							*end = new NFALookBehindNode("", true);
							return *end;
						case '-':
						default:
							raiseError();
						...
						switch ( pattern[ curInd ] ) {
						case 'i':
							flags |= RegEx::Pattern::CASE_INSENSITIVE;
							break;
						case 'd':
							flags |= RegEx::Pattern::UNIX_LINE_MODE;
							break;
						case 'm':
							flags |= RegEx::Pattern::MULTILINE_MATCHING;
							break;
						case 's':
							flags |= RegEx::Pattern::DOT_MATCHES_ALL;
							break;
						case ':':
							done = true;
							break;
			...
			if (noncap)
				cur = start = new NFAGroupHeadNode(grc);
			else
				cur = start = new NFASubStartNode;
		}
		else cur = start = new NFAGroupHeadNode(grc);
	}
	else {
		cur = start = new NFASubStartNode;
	}
	...
	while (curInd < pattern.getLength()) {
		...
		switch ( ch ) {
		case '^':
			if ((flags & Pattern::MULTILINE_MATCHING) != 0)
				next = new NFAStartOfLineNode;
			else
				next = new NFAStartOfInputNode;
			break;
		case '$':
			if ((flags & Pattern::MULTILINE_MATCHING) != 0)
				next = new NFAEndOfLineNode;
			else
				next = new NFAEndOfInputNode(0);
			break;
		case '|':
			--groupCount;
			cur->next = new NFAAcceptNode;
			cur = start = new NFAOrNode(start, parse(inParen, 1));
			break;
		case '\\':
			if (curInd < pattern.getLength()) {
				Bool eoi = 0;
				...
				case '9':
					...
					break;
				case 'A':
					++curInd; next = new NFAStartOfInputNode;
					break;
				case 'B':
					++curInd; next = new NFAWordBoundaryNode(0);
					break;
				case 'b':
					++curInd; next = new NFAWordBoundaryNode(1);
					break;
				case 'G':
					++curInd; next = new NFAEndOfMatchNode;
					break;
				case 'Z':
					eoi = 1;
				case 'z':
					++curInd; next = new NFAEndOfInputNode(eoi);
					break;
				default:
					t = parseEscape(inv, quo);
					if (!quo)
					{
						...
						{
							if ((flags & RegEx::Pattern::CASE_INSENSITIVE) != 0)
								next = new NFACIClassNode(t, inv);
							else
								next = new NFAClassNode(t, inv);
						}
						else
						{
							next = new NFACharNode(t[0]);
						}
					}
					else
					{
		...
		case '[':
			...
			else
			{
				...
				Buffer s = parseClass();
				for (int i = 0; i < (int)s.getLength(); ++i) clazz->vals[tolower(s[i])] = 0xFF;
				next = clazz;
			}
			break;
		case '.':
			{
			Bool useR = ( ( flags & UNIX_LINE_MODE  ) != 0 )
			         || ( ( flags & DOT_MATCHES_ALL ) != 0 );
			Bool useN = ( ( flags & DOT_MATCHES_ALL ) != 0 );
			NFAClassNode * clazz = new NFAClassNode( true );
			if ( ! useN )
				clazz->vals[ ( Uchar )'\n'] = 0x00;
			if ( ! useR )
		...
		case ')':
			...
			{
				--curInd;
				cur = cur->next = new NFAAcceptNode;
				flags = oldFlags;
				return start;
			}
			else
			{
				if (ahead)
				{
					cur = cur->next = new NFAAcceptNode;
					flags = oldFlags;
					return *end = new NFALookAheadNode(start, pos);
				}
				else if (indep)
				{
					cur = cur->next = new NFAAcceptNode;
					flags = oldFlags;
					return *end = new NFAPossessiveQuantifierNode( start, 1, 1);
				}
				else // capping or noncapping, it doesnt matter
				{
					*end = cur = cur->next = new NFAGroupTailNode(grc);
		...
		default:
			if ((flags & RegEx::Pattern::CASE_INSENSITIVE) != 0)
				next = new NFACICharNode(ch);
			else
				next = new NFACharNode(ch);
			break;
		}
		if (next)
		{
	...
	{
		if (inOr) cur = cur->next = new NFAAcceptNode;
		if (end) *end = cur;
	}
...
BufferArray RegEx::Pattern::split(
...
BufferArray RegEx::Pattern::findAll(
	const	Buffer &	pattern,
	const	Buffer &	str,
	const	Uint32		mode ) {
...
void RegEx::Pattern::registerPattern(
	const	Buffer &	name,
	const	Buffer &	pattern,
	const	Uint32		mode ) {
	...
	registeredPatterns[ name ] = RegPatternPair( pattern, mode );

}

void RegEx::Pattern::unregisterPatterns() {

	registeredPatterns.kill();

}

// ============================================================================

Buffer RegEx::Pattern::findNthMatch(
	const	Buffer &	pattern,
	const	Buffer &	str,
...
Buffer RegEx::Pattern::findNthMatch(
	...
	p.getMatcher()->setString( str );

	while ( i < ( int )matchNum && p.getMatcher()->findNextMatch() ) {
		++i;
	}

	if ( i == ( int )matchNum && p.getMatcher()->getStartingIndex() >= 0 ) {
		ret = p.getMatcher()->getGroup( 0 );
		startPos = ( Uint32 )p.getMatcher()->getStartingIndex();
	}
...
BufferArray RegEx::Pattern::split(const Buffer & str, const Bool keepEmptys, const Uint32 limit)
{
	Uint32 lim = (limit == 0 ? ( Uint32 )MAX_QMATCH : limit);
	int li = 0;
	BufferArray ret;

	getMatcher()->setString(str);

	while ( getMatcher()->findNextMatch() && ret.getSize() < lim ) {
		if (getMatcher()->getStartingIndex() == 0 && keepEmptys)
			ret += "";
		if ((getMatcher()->getStartingIndex() != getMatcher()->getEndingIndex()) || keepEmptys)
		{
			if (li != getMatcher()->getStartingIndex() || keepEmptys)
			{
				ret += str(li, getMatcher()->getStartingIndex() - li);
			}
			li = getMatcher()->getEndingIndex();
		}
	}
...
BufferArray RegEx::Pattern::findAll(const Buffer & str)
{
	getMatcher()->setString(str);
	return getMatcher()->findAll();
}
Bool RegEx::Pattern::matches(const Buffer & str)
{
	getMatcher()->setString(str);
	return getMatcher()->matches();
}
Uint32 RegEx::Pattern::getFlags() const
{
	return flags;
}
Buffer RegEx::Pattern::getPattern() const
{
	return pattern;
}

RegEx::Matcher * RegEx::Pattern::createMatcher(const Buffer & str)
{
...
int RegEx::NFACharNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
	if (curInd < str.getLength() && str[curInd] == ch) return next->match(str, matcher, curInd + 1);
	return -1;
}

// ============================================================================

RegEx::NFACICharNode::NFACICharNode(
	const	char		c ) {
...
int RegEx::NFACICharNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
	if (curInd < str.getLength() && tolower(str[curInd]) == ch) return next->match(str, matcher, curInd + 1);
	return -1;
}

// ============================================================================

int RegEx::NFAStartNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
	...
	while ((ret = next->match(str, matcher, ci)) == -1 && ci < (int)str.getLength())
	{
		matcher->clearGroups();
		matcher->starts[0] = ++ci;
	}
...
RegEx::NFAQuantifierNode::NFAQuantifierNode(
		NFANode *	internal,
	const	Uint32		minMatch,
	const	Uint32		maxMatch ) {

	inner = internal;
	inner->next = new NFAAcceptNode;
	min = Math::getMax( ( Uint32 )Pattern::MIN_QMATCH, minMatch );
	max = Math::getMin( ( Uint32 )Pattern::MAX_QMATCH, maxMatch );

}

int RegEx::NFAQuantifierNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const {
...
int RegEx::NFAGreedyQuantifierNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAGreedyQuantifierNode::matchInternal(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd,
...
int RegEx::NFALazyQuantifierNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
	int j, m = NFAQuantifierNode::match(str, matcher, curInd);
...
int RegEx::NFAPossessiveQuantifierNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
RegEx::NFAClassNode::NFAClassNode(
	const	Bool		invert ) {
...
RegEx::NFACIClassNode::NFACIClassNode(
	const	Bool		invert ) {
...
int RegEx::NFACIClassNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const {
...
int RegEx::NFASubStartNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFACIQuoteNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const {
...
int RegEx::NFALookAheadNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFALookBehindNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAStartOfLineNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAEndOfLineNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAReferenceNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAStartOfInputNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAEndOfInputNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAWordBoundaryNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAEndOfMatchNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAGroupHeadNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAGroupTailNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAGroupLoopPrologueNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAGroupLoopNode::match(
	const	Buffer &	str,
		Matcher *	matcher,
	const	Uint32		curInd ) const
{
...
int RegEx::NFAGroupLoopNode::matchGreedy(const Buffer & str, Matcher * matcher, const Uint32 curInd) const
{
...
int RegEx::NFAGroupLoopNode::matchLazy(const Buffer & str, Matcher * matcher, const Uint32 curInd) const
{
...
int RegEx::NFAGroupLoopNode::matchPossessive(const Buffer & str, Matcher * matcher, const Uint32 curInd) const
{
	int o = matcher->groupIndeces[gi];						// save info for backtracking
