...
void UUIdSerializer::doSerialize(
	const	UUId			& uuid,
		IO::ByteOutputStreamPtr	& ostream ) {

	ostream->putBytes( Buffer( uuid.getMemPtr(), 16 ) );

}

UUId UUIdSerializer::unSerialize(
		IO::ByteInputStreamPtr	& istream ) {

	UUId res;

	res.fromCompactBuffer( istream->getBytes( 16 ) );
