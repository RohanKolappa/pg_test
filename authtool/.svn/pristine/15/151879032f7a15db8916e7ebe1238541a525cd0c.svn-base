...
if (nArgs >= 2)
{
	strDirPath = WScript.Arguments(0);
	nSelectedIndex = WScript.Arguments(1);

	bDispList = false;
	bSelected = true;
	for (var i = 2; i < nArgs  ; i++ )
	{
		strArgsForExe = strArgsForExe + "\"" +  WScript.Arguments(i) + "\"" + " ";
	}
}
else if (nArgs == 1)
{
...
{
	WScript.Echo("Usage: ShowMediaList.Js <input dir> <Selected clip no.>(Optional) <Arguments> (for IPVSTransocde.exe)");
	WScript.Echo("Ex: ShowMediaList.js C:\Dir 4 -o output.y4m --fps 35 ..etc");
	WScript.Quit();
}

DisplayClipsList();

if ((nSelectedIndex > nIndexForURL) && (nSelectedIndex != -1))
{
	WScript.Echo(" Wrong clip number selected :" + nSelectedIndex + ".");
	bSelected = false;
}

/*while (!bSelected)
{
	WScript.StdOut.Write("Select the clip no:  ");
	nSelectedIndex = WScript.StdIn.ReadLine();

	if (nSelectedIndex <= nIndexForURL)
	{		
		bSelected = true;
	}
	else
	{
		WScript.StdOut.Write("Plase select in between 1 and " + nIndexForURL + ".\n" );
		bSelected = false;
	}
}*/
...
if (bSelected)
{
	WScript.Echo("Selected media URL is :");
	WScript.Echo(strURLArray[nSelectedIndex - 1]);

	var WshShell = new ActiveXObject("WScript.Shell");
	var command = "IPVSTranscode.exe -i" + "\"" + strURLArray[nSelectedIndex - 1] + "\"" +  " " + strArgsForExe ;
	var oExec    = WshShell.Run(command, 1, true);
...
function DisplayClipsList () 
{
	...
	var bDirExists = fileSystemObj.FolderExists(strDirPath);
	if (!bDirExists)
	{
		WScript.Echo("Folder " + strDirPath +" not found.\n");
		WScript.Quit();
	}

	var bDbExists = fileSystemObj.FolderExists(strDirPath + "\\db");
	if (!bDbExists)
	{
		WScript.Echo("Folder " + strDirPath + "\\db not found. \n");
		WScript.Quit();
	}

	var strDbFolder = fileSystemObj.GetFolder(strDirPath + "\\db");
	
	var eSubFiles = new Enumerator(strDbFolder.Files);
	
	if (bDispList)
	{
		WScript.Echo("GROUPNAME \t\t     DATE \t\t LENGTH ");
	}

	for(; !eSubFiles.atEnd(); eSubFiles.moveNext()) 
	{
		var fileName = eSubFiles.item().Name;
		var stream = parseXML(strDirPath + "\\db\\" + fileName, fileSystemObj);
		var clips = stream.getElements("CLIPS");
	    ...
	    if(clips.length > 0) 
		{
			if(clips[0].getText() != "") 
			{
				var mediaentry = PHP_Unserialize(stream.getElements("METADATA")[0].getText());
				var groupName = mediaentry[2];
				
				var clipfnames = clips[0].getText().split(",");
				var fnamePrefix = fileName.split("_")[0];
				var bDisGroupName = true;

				for (var clipCount=0; clipCount<clipfnames.length; clipCount++) 
				{
					var clipfname = fnamePrefix + "_" + clipfnames[clipCount];
					var clipDATA = "";
					
					clipDATA = parseXML(strDirPath + "\\db\\" + clipfname, fileSystemObj);
					var mediaClip = PHP_Unserialize(clipDATA.getElements("METADATA")[0].getText());
					var nLen = mediaentry[4];
					var hr = parseInt(nLen/3600);
					var mins = parseInt(nLen/60);
					var mm = mins % 60;
					var ss = nLen % 60;
					if(hr<10)
						hr = "0" + hr;
					...
					var nStreamLength =  hr + ":" + mm + ":" + ss;
					
					var dt = new Date(mediaentry[8] * 1000);
					...
					var strDate = MM + "/" + DD + "/" + dt.getFullYear() + " " + HH + ":" + MIN + ":" + SEC;
					
					if (mediaClip[11] == 1) //Media Type if it is V2D it should be 1.(MPEGTS-2, RTP-3, UDP-4)
					{
						if ((bDisGroupName) && (bDispList))
						{
							WScript.Echo("\n");
							WScript.Echo(groupName + "\t\t" + strDate + "\t" + nStreamLength );
							bDisGroupName = false;
						}

						var clipPath = strDirPath + "\\blob\\" + fnamePrefix + "_" + mediaClip[6];
						bDirExists = fileSystemObj.FolderExists(clipPath);
						if (!bDirExists)
						{
							WScript.Echo(clipPath + "Folder not found.\n");
							WScript.Quit();
						}

						strURLArray[nIndexForURL] = clipPath;
						nIndexForURL++;
						
						if (bDispList)
						{
							WScript.Echo( "    (" + nIndexForURL + ") " + mediaClip[2] );
						}
					}
				}
			}
...
function parseXML(fPath, objFSO) 
{
	...
	var strContents = objFSO.OpenTextFile(fPath, 1).ReadAll();
