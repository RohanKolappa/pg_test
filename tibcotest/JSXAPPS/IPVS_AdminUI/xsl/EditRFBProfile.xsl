<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="RFBMediaStreamProfile">
        <data jsxid="jsxroot">
        <record>
    ...
    <xsl:template name="record_attributes">
        ...
        <xsl:attribute name="RFBSDP-Template-Src">
            <xsl:value-of select="$record_el/Info/SDP-Template-Src"/>
        </xsl:attribute>
        <xsl:choose>
          <xsl:when test="$record_el/Info/RFBStreamConfig/IsMulticast='true'">
                <xsl:attribute name="RFBIsMulticast">1</xsl:attribute>
          </xsl:when>
          <xsl:otherwise>
                <xsl:attribute name="RFBIsMulticast">0</xsl:attribute>
          </xsl:otherwise>
        </xsl:choose> 
        <xsl:attribute name="RFBSDP-m-Template">
            <xsl:value-of select="$record_el/Info/RFBStreamConfig/SDP-m-Template"/>
        </xsl:attribute>
        <xsl:attribute name="RFBAVOption">
            <xsl:value-of select="$record_el/Info/RFBStreamConfig/AVOption"/>
        </xsl:attribute>
        <xsl:attribute name="RFBThumbnailEnable">
            <xsl:value-of select="$record_el/Info/RFBRecoderConfig/ThumbnailConfig/Enable"/>
        </xsl:attribute>
     </xsl:template>
</xsl:stylesheet>