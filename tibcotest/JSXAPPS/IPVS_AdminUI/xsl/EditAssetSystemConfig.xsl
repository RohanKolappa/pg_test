<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="record_attributes">
        ...
        <xsl:attribute name="DirtyBitblkBannerConfig">
            <xsl:value-of select="@DirtyBitblkBannerConfig"/>
        </xsl:attribute> 
        <xsl:attribute name="BannerText">
            <xsl:value-of select="$record_el/DeviceConfig/SystemElementsConfig/LoginBannerConfig/Banner"/>
        </xsl:attribute>
     </xsl:template>
</xsl:stylesheet>