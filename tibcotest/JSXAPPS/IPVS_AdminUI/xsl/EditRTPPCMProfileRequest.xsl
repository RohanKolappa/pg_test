<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="record">
        <RTP-PCMMediaStreamProfile>
          ...
          <Info>
            <SDP-Template-Src>
                <xsl:value-of select="@PCMSDP-Template-Src"/>
            </SDP-Template-Src>
            <RFBPCMStreamConfig>
              <IsMulticast>
                <xsl:choose>
                  <xsl:when test="@RTPPCMIsMulticast='1'">true</xsl:when>
                  <xsl:otherwise>false</xsl:otherwise>
                </xsl:choose> 
              </IsMulticast> 
              <SDP-m-Template>
                <xsl:value-of select="@PCMSDP-m-Template"/>
              </SDP-m-Template>
              <AVOption>
                <xsl:value-of select="@RTPPCMAVOption"/>
              </AVOption>
            </RFBPCMStreamConfig>
             <RTPPCMRecoderConfig>
                 <ThumbnailConfig>
