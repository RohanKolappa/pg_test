...
jsx3.Class.defineInterface("com.ipvs.api.SessionInfo", null, function(SessionInfo, SessionInfo_prototype) {
  ...
  /*
   * Gets a GrpFileType from the SessionInfo.
   * Typically not called directly. (called from get/add/update Entity).
   * @param {String} mediaGroupId
   * @return {String}
   */
  SessionInfo_prototype.getGrpFileType = function (mediaGrpId) {
    // get the GrpFile node
    var nodeGrpFile = this.selectSingleNode(".//GrpFile[@mediaGroupId='" + mediaGrpId + "']");
    if (!nodeGrpFile) return null;
    return nodeGrpFile.getAttribute("type");
  }
  /*
  ...
  SessionInfo_prototype._updateEntityNode = function(nodeEntity, objEntity) {
    ...
    if( nodeEntityUser ) {
      if( objEntity.userNID ) nodeEntityUser.setAttribute("resourceNID", objEntity.userNID );
      if( objEntity.userTitle ) {
        nodeEntityUser.setAttribute("resourceTitle", objEntity.userTitle );
    ...
    if( nodeEntityPVR ) {
      nodeEntityPVR.setAttribute("enable", this.getPVREnable());
      nodeEntityPVR.setAttribute("trimLength", this.getPVRTrimLength());
    }
  ...
  SessionInfo_prototype.getEntityObj = function(entityId) {
    ...
    if (objEntity.type == "PlyFile")
      objEntity.grpFileType = this.getGrpFileType(getNodeValue(nodeEntity.selectSingleNode("./File/@mediaGroupId"), ""));
...
//    return connId;
  ...
  SessionInfo_prototype._updateConnNode = function(nodeConn, srcEntityId, dstEntityId) {
    ...
    if (srcEntityIdFlag) {
      ...
      var connName = nodeConn.getAttribute("id");     
      var srcEntityName = this.getEntityName(srcEntityId);
  ...
  /*
  * Validates that atleast one destination supports trickplay
  */
  SessionInfo_prototype.isDestTrickPlayAvailable = function () {
    var trickPlay = this.selectSingleNode(".//Entity[@type='" + SessionInfo.ENTITYTYPE_DSTPORT + "']/Port[@trickPlay='true']");
    return (!jsx3.util.strEmpty(trickPlay)) ? true : false;
  }

  /*
  * Validates that atleast one source supports fftrack
  */
  SessionInfo_prototype.isSrcFFTrackAvailable = function () {
    var isPVRFFTrack = this.selectSingleNode(".//Entity[@type='" + SessionInfo.ENTITYTYPE_SRCPORT + "']/PVR[@ffTracks!='']");
    var isPlayBackFFTrack = this.selectSingleNode(".//Profile[@ffTracks!='']");
    return (!jsx3.util.strEmpty(isPVRFFTrack) || !jsx3.util.strEmpty(isPlayBackFFTrack)) ? true : false;
  }
