<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="UDPMediaStreamProfile">
        <data jsxid="jsxroot">
        <record>
    ...
    <xsl:template name="record_attributes">
        ...
        <xsl:choose>
          <xsl:when test="$record_el/Info/UDPStreamConfig/IsMulticast='true'">
                <xsl:attribute name="UDPIsMulticast">1</xsl:attribute>
          </xsl:when>
          <xsl:otherwise>
                <xsl:attribute name="UDPIsMulticast">0</xsl:attribute>
          </xsl:otherwise>
        </xsl:choose> 
     </xsl:template>
</xsl:stylesheet>