  ...
  FileDialog_prototype._redrawFileList = function() {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var record = {
        ...
        jsximg: file.isReadOnly() ? (bFolder ? "jsxapp:/images/plugin-icons/icon_7l.gif" : "jsxapp:/images/plugin-icons/icon_29l.gif") : (bFolder ? "jsxapp:/images/plugin-icons/icon_7.gif" : "jsxapp:/images/plugin-icons/icon_29.gif"),
        name: file.getName(),
...
if (jsx3.app.Browser.macosx) {
  ...
  FileDialog_prototype._redrawPathSelect = function() {
    ...
    while (file != null && this._checkSecurity(file)) {
      ...
      record.setAttribute("jsximg", file != null ? "jsxapp:/images/plugin-icons/icon_7.gif" : "jsxapp:/images/plugin-icons/icon_103.gif");
  ...
  FileDialog_prototype._redrawPathSelect = function() {
    ...
    while (file != null && this._checkSecurity(file)) {
      ...
      file = file.exists() ? file.isRoot() ? null : file.getParentFile() : null;
      record.setAttribute("jsximg", file != null ? "jsxapp:/images/plugin-icons/icon_7.gif" : "jsxapp:/images/plugin-icons/icon_102.gif");
    ...
    if (this.roots != null) {
      var currentRoot = this._current.isRoot() ? this._current : this._current.getRootDirectory() || this._current;
      for (var i = 0; i < this.roots.length; i++) {
        var root = this.roots[i];
        if (! currentRoot.equals(root)) {
          var node = this._getRecordFromFile(objXML, root);
          node.setAttribute("jsximg", "jsxapp:/images/plugin-icons/icon_102.gif");
...
//    window.alert("doPathSelect " + strRecordId);
  ...
  FileDialog_prototype.doNfCreate = function(folderName) {
    ...
    if (!newFolder.getParentFile().isRoot() && ! this._checkIsChild(newFolder)) {
  ...
  /** @private */
  FileDialog_prototype.doExecute = function(bConfirmed, selType) {
    var list = this._getFileList();
    ...
    if (toMoveTo != null) {
      ...
      //added this to change the Dir selection behaviour. Task#19192
      //OnChoose:Select the current directory instead of drill-down
      //On double click drill-down for sub directories of that directory
      //@selType:: null(Default) || Current(in case of click on onChoose) || Drill-Down.
      if ( this._type == FileDialog.TYPE_OPEN_FOLDER && selType == "Current" ) {
        toExec = this._current;
        this.doClose();
        if (this._fctchoose)
