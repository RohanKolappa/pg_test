<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="GetPortResponse/Port">
    <record>
      ...
      <xsl:apply-templates select="ConnectionStatus/PortSessionInfo/PortConnectionInfo"></xsl:apply-templates>
    </record>
  </xsl:template> 
  ...
  <xsl:template name="record_attributes">
    ...
    <xsl:choose>
      <xsl:when test="$record_el/@state='Ready'">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:attribute name="jsxthumbnailURL">JSXAPPS/IPVS_ClientUI/images/icons/srcthumb-online.png</xsl:attribute>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:when test="$record_el/@state='Unmanaged'">
        <xsl:attribute name="jsxthumbnailURL">JSXAPPS/IPVS_ClientUI/images/icons/srcthumb-unmanaged.png</xsl:attribute>
      </xsl:when>
      <xsl:otherwise>
        <xsl:attribute name="jsxthumbnailURL">JSXAPPS/IPVS_ClientUI/images/icons/srcthumb-offline.png</xsl:attribute>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:choose>
      <xsl:when test="ConnectionStatus/PortSessionInfo">
        <xsl:attribute name="jsxsessionstatus">JSXAPPS/IPVS_ClientUI/images/icons/icon_network_on.png</xsl:attribute>
        <xsl:attribute name="jsxbtnsessionstatus">JSXAPPS/IPVS_ClientUI/images/icons/connected_off.png</xsl:attribute>
      </xsl:when>
      <xsl:otherwise>
    ...
    <xsl:for-each select="ConnectionStatus/PortSessionInfo/PortConnectionInfo">
      <xsl:choose>
    ...
    <xsl:attribute name="jsxviewfrom">
      <xsl:value-of select="$record_el/Permissions/@viewfrom"/>
    </xsl:attribute>
    <xsl:choose>
      <xsl:when test="$record_el/Permissions/@viewfrom = 'false'">
