...
public class FileTransferNegotiator {
    ...
    private static final Map<XMPPI, FileTransferNegotiator> transferObject =
            new ConcurrentHashMap<XMPPI, FileTransferNegotiator>();
     ...
     */
    public static void setServiceEnabled(final XMPPI connection,
            final boolean isEnabled) {
        IPVSServiceDiscoveryManager manager = connection.getIPVSServiceDiscoveryManager();
     ...
     */
    public static boolean isServiceEnabled(final XMPPI connection) {
        for (String ns : NAMESPACE) {
            if (!connection.getIPVSServiceDiscoveryManager().includesFeature(ns))
                return false;
        }
        return true;
    }
    ...
    private FileTransferNegotiator(final XMPPI connection) {
...
//    private void configureConnection(final XMPPI connection) {
...
//    }
//
//    private void cleanup(final XMPPI connection) {
...
//    }
    ...
    public void cleanup(final XMPPI connection) {
    	  if (transferObject.remove(connection) != null) {
    	      byteStreamTransferManager.cleanup();
    	      inbandTransferManager.cleanup();
    	  }
    ...
    public StreamNegotiator selectStreamNegotiator(
            FileTransferRequest request) throws Exception {
     ...
     */
    public String getNextStreamID(String userID) {
        StringBuilder buffer = new StringBuilder();
        buffer.append(STREAM_INIT_PREFIX);
        buffer.append(userID.replace(".", "_").replace("@", "_").replace("/", "_"));
    ...
    public StreamNegotiator negotiateOutgoingTransfer(final String userID,
            ...
            final String desc, int responseTimeout) throws Exception {
