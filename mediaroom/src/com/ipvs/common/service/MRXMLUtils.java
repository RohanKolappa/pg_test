...
import com.barco.utils.json.JSONObject;
import com.barco.utils.json.XML;
...
public final class MRXMLUtils {
    ...
    public static  Document getDocument(String xml, String xsdURL) throws Exception {
       /* SAXBuilder builder = MRXMLUtils.getDocumentBuilder();
        builder.setFeature("http://xml.org/sax/features/validation", true);
        builder.setFeature("http://apache.org/xml/features/validation/schema", true);
        builder.setFeature("http://apache.org/xml/features/validation/schema-full-checking", true);
        builder.setProperty("http://apache.org/xml/properties/schema/external-noNamespaceSchemaLocation", xsdURL);
    ...
    public static Document loadXMLResource(String xmlFilePath, MRLog log) throws Exception {
        ...
        try {
        	//check for resource locally.
            url = MRDB.class.getResource(xmlFilePath);
        } catch (Exception ex) {
		...
		try {
			if (url != null) {
				...
				// resource not find locally check for remote resource
				doc = BundleUtils.loadBundleXMLResource(xmlFilePath);
			}
		} catch (Exception ex) {
...
//    }
    ...
    public static String getServiceVersion(MRLog log) throws Exception {
        if (SERVER_VERSION == null) {
            ...
            Document doc = null;
            try {
                url = MRDB.class.getResource(SERVER_VERSION_XML);
            } catch (Exception ex) {
    ...
    public static void nameSpaceRemoveNodeXpath(Node node, String name) throws Exception {
    	...
    	if(node.getNodeType()== Node.ELEMENT_NODE){
    		...
    		List<?> attributeList = ((Element)node).attributes();
    		nodeList = ListUtils.union(nodeList, attributeList);
    	  	if(nodeList != null && !nodeList.isEmpty()) {
    	    	for(Object object:nodeList) {
    				Node node1 = (Node)object;
    				if(node1.getNodeType() == Node.NAMESPACE_NODE || node1.getNodeType() == Node.TEXT_NODE)  {
    					continue;
    				}
    ...
    public static String nameSpaceGetValueNodeXpath(Node node, String name,String defaultValue) throws Exception {

    	Node newNode = nameSpaceGetNode(node, name);
    	if(newNode==null)
    		return defaultValue;
    ...
    public static void nameSpaceGetNodeValue(Node node, String name, List<Node> returnNodeList) throws Exception {
    	...
    	if(node.getNodeType() == Node.ELEMENT_NODE){
    		...
    		if(nodeList != null && !nodeList.isEmpty()) {
    			for(Object object:nodeList) {
    				Node node1 = (Node)object;
    				nameSpaceGetNodeValue(node1, name, returnNodeList);  				    			}
    		}    		
    	}
    }
    
    
    private static Node nameSpaceGetNode(Node node, String name) throws Exception {
    	...
    	if(node.getNodeType()== Node.ELEMENT_NODE){
    		...
    		if(nodeList != null && !nodeList.isEmpty()) {
    			for(Object object:nodeList) {
    				Node node1 = (Node)object;
    				Node newNode = nameSpaceGetNode(node1, name);
    ...
    public static void nameSpaceSetValueNodeXpath(Node node, String name,String value) throws Exception {
    ...
    public static void setServiceVersion() throws Exception {
		try {
				Document doc = BundleUtils.loadBundleXMLResource(SERVER_VERSION_XML);
				SERVER_VERSION = doc.getRootElement().attributeValue("Ver");
		} catch (Exception e) {
			throw new MRException(MRExceptionCodes.SERVICE_VERSION_XML_NOT_VALID, MRXMLUtils.generateErrorXML("error", "loadXMLResource url '" + SERVER_VERSION_XML + "' failed"));
  	   }
    ...
    public static String xmlToJSONString(String xmlStr, int indentVal){
    	/*JSONObject xmlJSONObj = XML.toJSONObject(xmlStr);
   		return xmlJSONObj.toString(indentVal);*/
   		JSONObject xmlJSONObj = XML.toJSONObject(xmlStr); 
			return xmlJSONObj.toString(indentVal); 
    }
