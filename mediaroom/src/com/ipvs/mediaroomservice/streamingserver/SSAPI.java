...
public abstract class SSAPI {
	...
	protected String getAuthData(Document streamDataDoc) throws Exception {
	    ...
	    if(!"".equals(destStreamURL)) // Is UnManaged Dest 
	    	return "";
	    
		String streamURL = MRXMLUtils.getValueXpath(streamDataDoc, "//streamURL", "");
	    if(!"".equals(streamURL)) { // Is UnManaged Source
	    
	    	// Extract the token from the URL and return it
	    	String [] url = streamURL.split("\\?authdata=");    	
	    	String authData = url[url.length-1].split("=")[0].split("\\?")[0];
	    	return authData;
