...
public class ResumeExportImportFileRequestHandler implements MRRequestHandlerI {
    ...
    public Document getResponse(MRRequest request, XMPPI xmpp, MRLog log, MRDB db, MRResourceFactoryI resourceFactory, ServiceAgentFactoryI serviceAgentFactory) throws Exception {
        ...
        String result = FileSMQueue.getInstance().resume(request, xmpp, log, db, resourceFactory, serviceAgentFactory, null);
        if (result != null) {
            MRXMLUtils.addElement(respRoot, "Result", result);
        }
        //return response
