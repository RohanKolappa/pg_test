...
public class UpgradeMediaRequestHandler implements MRRequestHandlerI {
    ...
    public Document getResponse(MRRequest request, XMPPI xmpp, MRLog log, MRDB db, MRResourceFactoryI resourceFactory, ServiceAgentFactoryI profileFactory)
        ...
        //Upgrade Media Store
        mediaStoreServer.upgradeMediaStore(mediaStoreFileNID, mediaStreamFileDoc, rootDir, revision);
        MediaStoreServiceUtils.updateRevision(mediaStreamFileDoc, revision);
        MediaStoreServiceUtils.updateMediaStreamFilePermissions(mediaStreamFileDoc, mediaStreamFileNID, request, xmpp, log, db, resourceFactory, profileFactory);
        log.addInfo("Upgrade Media Store. MediaStoreFileNID:= "+mediaStoreFileNID,MRLog.INFO,MRLog.NOTIFY);
