...
public class UpdateDestFileMetaDataRequestHandler implements MRRequestHandlerI {
    ...
    public Document getResponse(MRRequest request, XMPPI xmpp, MRLog log, MRDB db, MRResourceFactoryI resourceFactory, ServiceAgentFactoryI serviceAgentFactory)
        ...
        if (fileNID.isEmpty()) {
            String recordID = MRXMLUtils.getValueXpath(requestDoc, "//MetaData/Tag[@Name='ID']", "");

            if (!recordID.isEmpty()) {
                fileNID = MediaRoomServiceUtils.getMediaStreamFileResourceNID(recordID, request, request.getUserJID(), xmpp, log, db, resourceFactory, serviceAgentFactory);
            }
        }
        ...
        try {
            ...
            exp.printStackTrace();
            throw new MRException(MediaRoomServiceExceptionCodes.UPDATE_METADATA_FAILED, exp.toString());
        }
    }
}
