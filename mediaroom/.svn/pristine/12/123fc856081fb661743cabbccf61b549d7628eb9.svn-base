...
public class MCastIPAddressReservation extends Reservation{
    ...
    public void updateReservationForReliability(String inputInstanceID,String outputInstanceID,String ipAddress) throws Exception {
    	if(isUseDefault()) {
    		return;
    	}
    	if(!inputMCastIPAddress.containsKey(inputInstanceID)) {
    		super.updateReservationForReliability(NetworkResource.ipAddresstoInt(InetAddress.getByName(ipAddress)));
    ...
    public void release(String outputInstanceID) {
    	try {
    		...
    		System.out.println("Release OuputInstance :="+outputInstanceID);
    		if (!outputMCastIPAddress.containsValue(ipAddress)) {
    			...
    			System.out.println("Release inputInstance :="+inputInstanceID);
    		}
    	}catch(Exception e) {
    		e.printStackTrace();
    	}
