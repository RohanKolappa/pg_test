...
public class StreamTable {
	...
	public StreamTable(String userJID, String deviceNID, String deviceType,
		StreamingServerI streamingServer, MRLog log, XMPPF xf) {
...
//    	}
	...
	public Element setupStreamElement(String streamNID, Document streamDataDoc, Document mediaSourceDataDoc, String actionNID)
	    ...
	    long t = xf.xmpp(userJID).getCurrentTimeMillis();
	...
	private void createRelayRoom(String sourceNID, long offset, long startTimecode, long syncTC,
	    ...
	    MRXMLUtils.setValueXpath(requestDoc, "//"+MRConst.CLIENT_DATA,inputNID);
	    ...
	    String roomNID = null;
	    String agentJID =  AppServerAgentUtils.getAppServerAgentJID(userJID,xf.sf(userJID));
	    ...
	    ServiceAgentUtils.sendAgentRequestMessage(xf.xmpp(userJID), log, fromAgentClassName, roomNID, agentJID, agentClassName, requestDoc, info, requestNID);
	}
	private void deleteRelayRoom(String roomNID) throws Exception {
	    ...
	    String requestNID = "none";
	    ServiceAgentUtils.sendAgentRequestMessage(xf.xmpp(userJID), log, fromAgentClassName, null, agentJID, agentClassName, requestDoc, info, requestNID);
	}
	...
	private boolean checkResponseOK(Document responseDoc,MRClient client, Element inputElement) {
		try {
			String stateStr = MRXMLUtils.getAttributeValueXpath(responseDoc, "//Header",MRConst.STATE, "");
			int state = 0;
			try {
	...
	private void handleAddStreamActionResponse(String fromJID, Document responseDoc,MRClient client) throws Exception {
		String inputNID = MRXMLUtils.getValueXpath(responseDoc, "//"+MRConst.CLIENT_DATA, "");
	...
	private void handleCreateRelayConnectionResponse(String fromJID, Document responseDoc,MRClient client) throws Exception {
	    ...
	    MRXMLUtils.addAttribute(inputElement,"state", String.valueOf(StreamTable.INPUT_RELAY_WAITFORADDSTREAMACTIONRESPONSE));	    
	
	    com.ipvs.common.XMPPTransaction.startBlock("SourceTable.startRelayStream");
	
	    String agentJID =  AppServerAgentUtils.getAppServerAgentJID(userJID,xf.sf(userJID));
	    String agentClassName = AppServerAgent.class.getName();
	    String fromAgentClassName = SourceStreamAgent.class.getName();
	    String info = "SourceTable::handleResponse > Sending Request AddStreamAction";
	    String requestNID = "none";
	
	    String xmlFilePath = "/com/ipvs/mediaroomservice/xml/" + SourceStreamAgent.ADDSTREAMACTION_REQUEST_XML;
	    Document requestDoc = MRXMLUtils.loadXMLResource(xmlFilePath, log);
	    MRXMLUtils.getElementXpath(requestDoc, "//"+MRConst.CLIENT_DATA).setText(inputNID);
	    // Not setting the clientdata causes the response to be dropped
	    MRXMLUtils.getElementXpath(requestDoc, "//streamNID").setText(streamNID);
	    MRXMLUtils.getElementXpath(requestDoc, "//profileNID").setText("");
	    MRXMLUtils.getElementXpath(requestDoc, "//updateAction").setText(MediaRoomServiceConst.START);
	
	    ServiceAgentUtils.sendAgentRequestMessage(xf.xmpp(userJID), log, fromAgentClassName, null, agentJID, agentClassName, requestDoc, info, requestNID);
	    // MediaRoomServiceUtils.requestStreamAction(userJID, xmpp, log, db, resourceFactory, serviceAgentFactory, streamNID, MediaRoomServiceConst.START);
	...
	public void handleRelayInputStreamResponse(MRClient client, Document doc1, String action1, String streamNID, String streamURL, Document streamDataDoc)throws Exception {
        ...
        // Check if this is an event to be propogated
        if (MediaRoomServiceConst.SOURCEEVENT.equals(action1)) {
		    String actionData = MRXMLUtils.elementChildrenToString(MRXMLUtils.getElementXpath(doc1, "//sourceEventActionData"));
		    this.sendMessageToAllDestinations(client, doc1, MediaRoomServiceConst.SOURCEEVENT, actionData);
		    return;
		}

        Element response = MRXMLUtils.getElementXpath(doc1, "//Response");
        if(response == null)
        	return;
        ...
        try {
        	// int state = Integer.parseInt(MRXMLUtils.getValueXpath(doc1, "//Response/header/state", ""));
        	int state = Integer.parseInt(MRXMLUtils.getAttributeValueXpath(doc1, "//Header", "state", String.valueOf(MRRequest.STATE_EXCEPTION)));
		...
		Element startResponseData = MRXMLUtils.getElementXpath(doc1, "//StartResponseData");
        if( (startResponseData != null) && (inputState == INPUT_OK)) {
		    ...
		    String logData = null;
		    Document sourceEventDoc = StreamAgentUtils.createSourceEventMessage(streamNID, MediaRoomServiceConst.SOURCEEVENT, eventCode,
		            StreamTable.getStreamNIDXML(streamNID), 0, actionData, MRLog.OK, logData, log);
		    this.sendMessageToAllDestinations(client, sourceEventDoc, MediaRoomServiceConst.SOURCEEVENT, actionData);
        }
	}


	public void handleRelayInputStreamResponseOld(MRClient client, Document doc, String action, String streamNID, String streamURL, Document streamDataDoc)throws Exception {
		Element inputElement = MRXMLUtils.getElementXpath(this.sourceTableDoc, "//Input[@streamNID='" + streamNID + "']");
		// System.out.println("StreamTable:handleRelayInputStreamResponse inputElement=" + MRXMLUtils.elementToString(inputElement));
		if (inputElement == null) {
		    return;
		...
		if (MediaRoomServiceConst.START.equals(action) || MediaRoomServiceConst.SOURCE_STARTOUTPUT.equals(action) || MediaRoomServiceConst.SOURCE_SWITCHCURRENT.equals(action) ||
		    ...
		    String nowStr = String.valueOf(xf.xmpp(userJID).getCurrentTimeMillis());
	...
	private void sendMediaSourceEvent( 
		...
		for(Object o:mediaSourceList) {
            ...
            String logStr = "";
    		List<?> streamList = MRXMLUtils.getListXpath(mediaSource, ".//Stream");
    		for(Object s : streamList) {
    			Element stream = (Element)s;
    			String agentJID = stream.attributeValue("destAgentJID");
    			ServiceAgentUtils.sendAgentEvent(
            		client.getXmpp(),
            		client.getLog(), 
            		roomNID, 
            		MediaRoomServiceConst.SOURCESTREAMAGENT, 
            		agentJID, // MediaRoomServiceConst.WILDCARD, 
            		MRConst.LAYOUT_SERVICE, 
            		eventLevel, 
            		client.getUserJID(), 
            		eventWallclock, 
        ...
        while (objStreamListIterator.hasNext()) {       	
            ...
            if (streamDataDoc != null) {
                ...
                String info = "SourceStreamAgent:=" + client.getUserJID() + "/" + outputStreamNID + " > SourceEvent Message action=" + action + " to " +
                    ServiceAgentUtils.getAgentShortName(serviceName);
                ...
                ServiceAgentUtils.sendAgentEvent(client.getXmpp(),client.getLog(), outputRoomNID, "", agentJID, serviceName, eventLevel, client.getUserJID(), eventWallclock, MediaRoomServiceConst.SOURCEEVENT, response.getRootElement(), "", info);
            }
        }
    }
