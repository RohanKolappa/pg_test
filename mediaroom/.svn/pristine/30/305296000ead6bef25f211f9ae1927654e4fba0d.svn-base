...
public class Socks5TransferNegotiator extends StreamNegotiator {
    ...
    private final XMPPI connection;
    ...
    public InputStream createIncomingStream(StreamInitiation initiation) throws Exception {
	...
	private SelectedHostInfo selectHost(Bytestream streamHostsInfo)
        ...
        int timeout = OutgoingFileTransfer.getResponseTimeout() / 4;
        //System.out.println((new Date())+ " " + OutgoingFileTransfer.getResponseTimeout() + " timeout 1 " + timeout);
        while (it.hasNext()) {
            ...
            try {
                ...
                socket.bind(null);
                // Connect to with a timeout  
                socket.connect(new InetSocketAddress(address, selectedHost.getPort()), timeout);                
                //long diff = System.currentTimeMillis() - start;
                //System.out.println((new Date())+ " " + OutgoingFileTransfer.getResponseTimeout()+ " timeout 2 " + timeout +"  syn_sent timout diff " + diff);                                
    ...
    private String createDigest(final String sessionID, final String initiator,
        ...
        return StringUtils.hash(sessionID + initiator + target);
    }
    ...
    private Socket initBytestreamSocket(final String sessionID,
        ...
        Socks5TransferNegotiatorManager.ProxyProcess process=null;
        try {
            //if(!connection.getLoginType().equals(XMPPI.LOGIN_TYPE_COMP)){
    ...
    private SelectedHostInfo waitForUsedHostResponse(String sessionID,
            ...
            final Bytestream query) throws Exception
    {
        ...
        Packet packet = collector.nextResult(OutgoingFileTransfer.getResponseTimeout());
        collector.cancel();
        Bytestream response;
        ...
        if (used.getJID().equals(query.getFrom())) {
            ...
            IQ serverResponse = (IQ) collector.nextResult(OutgoingFileTransfer.getResponseTimeout());
            collector.cancel();
    ...
    public static List<String> discoverLocalIP() throws UnknownHostException {
    ...
    private Bytestream createByteStreamInit(final String from, final String to,
            final String sid, final List<String> localIP, final int port)
    {
        ...
        if (localIP != null && !localIP.isEmpty() && port > 0) {
        	for(String ipaddress:localIP) {
        		bs.addStreamHost(from, ipaddress, port);
        	}
        }
