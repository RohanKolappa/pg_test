...
public class FileTransferManager {
	 ...
	 * 
	 * @param connection
	 *            The XMPPI that the file transfers will use.
	 */
	public FileTransferManager(XMPPI connection) {
    ...
    public void cleanup(final XMPPI connection) {
  	  if (fileTransferNegotiator != null) {
  		fileTransferNegotiator.cleanup(connection);
  	  }
    }
	...
	public OutgoingFileTransfer createOutgoingFileTransfer(String userID) {
	    ...
	    if(userID.endsWith("."+connection.getServiceName())){
	        //its component    
	    } else if (StringUtils.parseName(userID).length() <= 0
		...
		return new OutgoingFileTransfer(connection.getUser(), userID,
				fileTransferNegotiator.getNextStreamID(connection.getUser()),
