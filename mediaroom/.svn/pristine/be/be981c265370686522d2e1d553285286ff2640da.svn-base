...
public class RepairMediaResponseHandler implements AgentResponseHandlerI {
    public void handleAgentResponse(MRResponseHandlerI responseHandler, MRRequest request, XMPPI xmpp, MRLog log, MRDB db, MRResourceFactoryI resourceFactory, ServiceAgentFactoryI serviceAgentFactory)
        ...
        try {
            ...
            mediaStreamFileDoc = MediaStoreServiceUtils.checkMediaStreamFileState(responseDoc, MediaStoreServiceExceptionCodes.REPAIR_REQUEST_FAILED, log, db, request);
            ...
            MRException me = new MRException(MediaStoreServiceExceptionCodes.REPAIR_REQUEST_FAILED, exp.toString());
            request.setException(me);
            logData = MRXMLUtils.generateErrorXMLTags("Code", String.valueOf(MediaStoreServiceExceptionCodes.REPAIR_REQUEST_FAILED), "Description", exp.toString());
            exp.printStackTrace();
        }

        request.setState(state);

        MRRequest mediaStreamFilerequest = MediaStoreServiceUtils.createMediaStreamFileRequest(request, responseDoc,  responseXMLEL, "RepairResponse", log);
