...
public class GetPolicyXPathRequestHandler implements MRRequestHandlerI {
    ...
    public Document getResponse(MRRequest request, XMPPI xmpp, MRLog log, MRDB schema, MRResourceFactoryI resourceFactory,
        ...
        String xPath = "";
        Element parentEL = MRXMLUtils.getElementXpath(reqDoc, "//PolicyManagementRequestData");

        List<?> operatorList = MRXMLUtils.getListXpath(parentEL, "//GetPolicyXPathRequest/OperatorList/Operator");
        ...
        matchingPolicyList = PEMImplementation.getMatchingPoliciesListBasedOnOperatorAndRole(operatorList,
                PolicyAdminConstants.POLICY_ADMIN_POLICY_OBJECTTYPE, action, roleObjectType, serviceName, request, schema, log);
        ...
        if ((matchingPolicyList != null) && (matchingPolicyList.size() != 0)) {
            ...
            xPath = PEMImplementation.getPolicyAdminXpath(policyList, operatorList,roleObjectType, serviceName, request, schema, log);
        } else {
