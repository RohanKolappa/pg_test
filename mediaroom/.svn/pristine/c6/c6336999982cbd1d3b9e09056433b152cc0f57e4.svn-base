...
public class ResumeRecordingRequestHandler  extends ProxyPauseResumeBaseRequestHandler implements MRRequestHandlerI {
    ...
    throws Exception {
    	log.addInfo("ResumeRecordingRequestHandler - Begin", MRLog.INFO,MRLog.NOTIFY);
		sendRecordingPauseResumeRequest (MediaRoomServiceConst.TRACK_CATCHUP, MediaStoreServiceConst.RECORDING_STARTED, ProxyExceptionCodes.RECORD_STARTED, ProxyConstants.RESUMERECORDINGREQUEST, request, xmpp,  log,  db,  resourceFactory, serviceAgentFactory);
		log.addInfo("ResumeRecordingRequestHandler - End", MRLog.INFO,MRLog.NOTIFY);
