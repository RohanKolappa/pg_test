...
STATIC TCFGSYSTEM SystemCfgs[] = 
{
	...
	{
		{
			"SAA7113 SECAM",					// name of configuration for system setting
			"SAA7113 SECAM",					// sensor configuration file name
			0,
			sizeof(TCFGSYSTEM),
		},
...
{
	SINT32 rt = SUCCESS;
	MatrixReload_Context_t* ctx = (MatrixReload_Context_t*) pContext;
...
{	
	SINT32 rt=-1, value1;
	SINT32 rt1=-1,i;
	UINT8 actual_value=200;		

	if ( value > 100 || value < 0 ) return E_ARG;

	value1 = value * 255 / 100;
/*The following code Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
	for (i=0;i<100;i++)
	{
		...
		while (rt1!=SUCCESS)	 
		rt1 = g_pAF->I2C_ReadRegister(Cookie, DeviceAddr, 8, SAA7113_ADDR_BRIGHTNESS, &actual_value, IM_I2C);
		if (value1==actual_value)
		break;	
	}
/*The above code is Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
	return rt;
...
/*The following code Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
	for (i=0;i<100;i++)
	{
		...
		while (rt1!=SUCCESS)	 
		rt1 = g_pAF->I2C_ReadRegister(Cookie, DeviceAddr, 8, SAA7113_ADDR_CONTRAST, &actual_value, IM_I2C);
		if (value1==actual_value)
		break;	
	}
/*The above code is Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
...
/*The following code Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
	...
	for (i=0;i<100;i++)
	{
		...
		while (rt1!=SUCCESS)	 
		rt1 = g_pAF->I2C_ReadRegister(Cookie, DeviceAddr, 8, SAA7113_ADDR_HUE, &actual_value, IM_I2C);
		if (value1==actual_value)
		break;	
	}
/*The above code is Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
...
/*The following code Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
	for (i=0;i<100;i++)
	{
		...
		while (rt1!=SUCCESS)	 
		rt1 = g_pAF->I2C_ReadRegister(Cookie, DeviceAddr, 8, SAA7113_ADDR_SATURATION, &actual_value, IM_I2C);
		if (value1==actual_value)
		break;	
	}
/*The above code is Modified for the bug: I2C_WriteRegister fails to write registers; Wiseway*/
...
{
	MatrixReload_Context_t* ctx = PIAllocMem(sizeof(MatrixReload_Context_t));
	...
	*pBoardContext = ctx;
	...
	strcpy(pBoardInfo->Name, "Matrix II Reloaded");
	pBoardInfo->RevisionNum = 0x0204;
