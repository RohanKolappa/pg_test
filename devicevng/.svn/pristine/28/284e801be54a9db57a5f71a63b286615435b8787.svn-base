...
public class XMPPTransaction {
	...
	public synchronized void waitForResponse(Logger log) throws Exception {
		this.waitForResponse(timeout, log);
	}
	
	public synchronized void waitForResponse(long timeout, Logger log) throws Exception {
		this.startTrace();
		...
		if(response==null || System.currentTimeMillis()- time_started > timeout)
		    log.info(id + " XMPPTransaction waitForResponse: "  + ( System.currentTimeMillis()- time_started) + " milisec.  is response null? " +(response==null) );
		this.endTrace();
	}
	...
	public synchronized Object getResponse(Logger log) throws Exception {
		if (exception != null) {
			StringWriter sw = new StringWriter();
			exception.printStackTrace(new PrintWriter(sw));
	        //added to debug
	        if(System.currentTimeMillis()- time_started > timeout)
	            log.info(id + " XMPPTransaction exception "   + sw.toString());
			throw new ServiceException(MRExceptionCodes.SYSTEMERROR, sw.toString());
		}
        //added to debug
        if(response==null || System.currentTimeMillis()- time_started > timeout)
            log.info(id + " XMPPTransaction getResponse: "  + (System.currentTimeMillis()- time_started ) +" milisec. is response null? " +(response==null)  );
		return response;
	}
