/*
 * vncsockets.c - deal with TCP & UDP sockets.
 *
...
int rfbMaxClientWait = 10000;   /* time (ms) after which we decide client has
...
int rfbConnect(rfbScreen, host, port)
    rfbScreenInfoPtr rfbScreen;
    char *host;
    int port;
{
#if 1// not used in netviz2
    return -1;
#else
    int sock;
    int one = 1;
...
rfbConnectToTcpAddr(host, port)
    ...
    int port;
{
#if 1  // not used with Netviz2 fake VNC server
    return -1;
#else
    struct hostent *hp;
    int sock;
	...
	addr.sin_addr.s_addr = *(uint32_t *)hp->h_addr;//*(unsigned long *)hp->h_addr;
    }
