...
var SessionCtrlZoomPanel = Class.extend({
  ...
  _build: function () {
    ...
    var child = jQuery('<div id="' + this.options.divId + '_zoomPopup' +
      ...
      '</ul><span class="zoomNotch"><span></div>').addClass('SessionCtrlZoomPanel');
  ...
  destroy: function(pluginName) {
    ...
    // destroy self
    this.$elem.find('.SessionCtrlZoomPanel').remove();
  ...
  popupZoom: function(pos, hide) {
    ...
    if (this.options.playerUsed === 'streamPlayerIPV') {
      var $elemForm = this.$elem.find('.SessionCtrlZoomPanel');
...
jQuery.plugin('sessionCtrlZoomPanel', SessionCtrlZoomPanel);
