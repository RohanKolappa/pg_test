...
function showList(list, listType) {
	...
	var total = 0;
	var viewportList = document.getElementById("viewportList");
  if(viewportList)
	  viewportList.parentNode.removeChild(viewportList);
	// Add the ports
	if(list instanceof Array) { // more than 1 port
    var mHTML = "<ul id='viewportList' data-role=\"listview\">";
	  for (var i = 0; i < list.length; i++) {
      var txt = list[i]["@title"];
      var itemId = getValue(listType, list[i]);
      mHTML += "<li id='" + itemId + "' onclick=\"handleMediaSelected('" + itemId + "', '" + txt + "')\">" + txt + "</li>";
    }
	  mHTML += "</ul>";
    total = list.length;
	  document.getElementById("selector").innerHTML = mHTML;
...
function handleMediaSelected(id, title) {
