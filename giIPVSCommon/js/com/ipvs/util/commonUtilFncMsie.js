...
function getHWInfo_InterfacesInfo_Msie_en() {
  ...
  try {
    ...
    var objStdOut = FSO.OpenTextFile(path, 1);
    while (!objStdOut.AtEndOfStream) {
      var strLine = objStdOut.ReadLine();
      if (strLine.search("PPP adapter") != -1) {
        ...
        for (var i = 0; i < 8; i++) {
          var subStrLine = objStdOut.ReadLine();
          if (subStrLine.search("Media disconnected") != -1) {
            //read th next 2 lines
            subStrLine = objStdOut.ReadLine();
            subStrLine = objStdOut.ReadLine();
            i = 9;
          }
          //for XP
...
function prefFileWrite_Msie(objPNameSpace, filePath, preferencesXMLNode) {
  var theFile = null;
  try {
    var passContentStr = preferencesXMLNode.toString();
    var FSO = new ActiveXObject("Scripting.FileSystemObject");
    theFile = FSO.OpenTextFile(filePath, 2, true);
    theFile.write(passContentStr);
    theFile.close();
  } catch (e) {
    promptUserSettings(e.number, "Preferences.xml failed to write to" + filePath + ": " + e.description);
  }
}
