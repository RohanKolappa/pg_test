	...
	if [ $bweb_auth_level -ge 1 ]; then
		case ${GET_id} in
			revert)
				...
				status="REVERT"
				/V2O/bin/revert_config -reset_network ${GET_reset_network} -delete_media ${GET_delete_media} 2>&1 >> /tmp/cmd.log
				;;
			shutdown)
			...
			updateedid)
				...
				;;
			rebuildDisk)
				echo "Rebuilding ... /V2O/bin/rebuild_disk" > /tmp/cmd.log
				status="OK"
				echo yes | /V2O/bin/rebuild_disk -diskid ${FORM_diskid} 2>&1 >> /tmp/cmd.log
				
				local errmsg=`grep -c 'Field' /tmp/cmd.log`
				if ( [ $errmsg != 0 ] ); then
					cmd_error_begin
						tail 1 /tmp/cmd.log | tr '\n' ' '
					cmd_error_end
					status="ERROR"
				fi
				;;
