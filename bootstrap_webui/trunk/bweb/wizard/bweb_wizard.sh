...
_bweb_getupdatedconfig() {
	...
	if [ ! -f /tmp/get_$1.xml ]; then
		if [ -f /V2O/bin/get_$1 ]; then
			/V2O/bin/get_$1  > /tmp/get_$1.xml
		else
#			Might be a list or table entry
			local entry=`echo $1 | sed -e 's/_entry$//'`
			if [ $entry != $1 ]; then
				if [ -f /V2O/bin/get_${entry}_table ]; then
					/V2O/bin/get_${entry}_table > /tmp/get_${entry}_table.xml
				elif [ -f /V2O/bin/get_${entry}_list ]; then
					/V2O/bin/get_${entry}_list > /tmp/get_${entry}_list.xml
				fi
			fi
		fi
	fi
