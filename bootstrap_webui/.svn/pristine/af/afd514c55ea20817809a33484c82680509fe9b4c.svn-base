...
bweb_locate_res() {
	...
	while [ $i -ge 0 ]; do
		...
		while [ $j -ge 0 ]; do
			...
			echo "bweb_localui res $res" >> /tmp/megm.log
	...
	echo "bweb_resource $bweb_resource" >> /tmp/megm.log
...
#		cmdcli=`echo $cmdcli | sed -e s'/set_if_list/mod_if_list 1/'`
		...
		cmd=""
		local x=`echo $envvar | tr ' ' '\n' |grep $i\_ |grep FORM |grep -v _cli_  | sed -e 's/ /_spaces_/g'`
