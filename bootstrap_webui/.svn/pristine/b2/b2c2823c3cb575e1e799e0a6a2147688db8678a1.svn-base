...
var barco_status_url = "/cgi-bin/localui/dms/get_status.cgi?cmd=";
...
function barco_NGSCheckIfRebooted(evt) {
  var pingUrl;
  var isDeviceCombo;
  isDeviceCombo = (document.getElementById('isdevicecombo').value=="Yes" ? true : false);
  
  if (isDeviceCombo && barco_ngs_status !== "REVERT")
...
function barco_processCmdData(id, self) {
  ...
  if (id === "upgrade") {
	showPrompt=true;
	confirmationMsg="This utility will allow you to upgrade software on this DMS. The DMS will reboot automatically. Proceed with software upgrade?";
	...
	var delete_media_checked = $('input#delete_media').prop('checked');
	data += '&delete_media=' + (delete_media_checked ? "yes" : "no");
	confirmationMsg="All settings will be lost except the network settings and media. If \"Reset network settings\" is checked then the DMS will revert back to a DHCP assigned IP address that this client may not be able to reach. If \"Delete Media\" is checked, all recorded media will be lost. Do you wish to continue?";
  }
...
function barco_sendAJAXCmdRequest(data) {
  ...
  $.ajax({  
    type: "GET",  
    url: "/cgi-bin/localui/dms/processcmd.cgi",
    // dataType: 'xml',
    data: data,  
...
function barco_setControlsState () {
	...
	var useNTP = $('input[name="dms_server_options_checkbox_enablentp"]');
	useNTP.on('change', changeNTPState);
	
	var autoDetect = $('input[name="dms_server_options_checkbox_autodetect"]');
	autoDetect.on('change', changeAutoDetectState);
	
	var deviceType = $('select#dms_server_options_deviceType');
	deviceType.on('change', changeDeviceType);
	changeDeviceType();
	
	if (deviceType.val() == "MS/DMS Combo")
	{
...
function changeAutoDetectState () {
	isAutoDetect= (document.getElementById('dms_server_options_checkbox_autodetect').checked ? true : false);
	$('input[name="dms_server_options_serverip"]').prop("disabled", isAutoDetect);
}

function changeNTPState() {
	useNTP= (document.getElementById('dms_server_options_checkbox_enablentp').checked ? true : false);
	$('input[name="dms_server_options_ntpip"]').prop("disabled", useNTP);
}


function changeDeviceType() {
	isCombo= (document.getElementById('dms_server_options_deviceType').value=="DMS" ? false : true);	

	var msIPInput= $('input[name="dms_server_options_serverip"]');
	var autoDetectCheckbox= $('input[name="dms_server_options_checkbox_autodetect"]');
	var enableNTPCheckbox= $('input[name="dms_server_options_checkbox_enablentp"]');
	var ntpIPInput= $('input[name="dms_server_options_ntpip"]');
	var enableDiscoveryCheckbox= $('input[name="dms_server_options_checkbox_enablediscovery"]');
	var enableUserRegistryCheckbox= $('input[name="dms_server_options_checkbox_enableuserregistry"]');
	
	autoDetectCheckbox.prop("disabled", isCombo);
	enableNTPCheckbox.prop("disabled", !isCombo);
	enableDiscoveryCheckbox.prop("disabled", !isCombo);
	enableUserRegistryCheckbox.prop("disabled", !isCombo);

	if(isCombo)
	{
		msIPInput.prop("disabled", true);
		enableNTPCheckbox.prop("disabled", false);
		changeNTPState();
	}
	else
	{
		ntpIPInput.prop("disabled", false);
		autoDetectCheckbox.prop("disabled", false);
		changeAutoDetectState();	
	}
}
...
function barco_showHideExitAdminLink() {
	...
	if (isDeviceCombo)
		...
		exitAdminField.style.display = "none";
}	