...
bweb_locate_res '/usr/share/bweb/localui/' 'bweb_private_localui' '.sh'
...
_bweb_auth_check() {
	...
	then
		...
		local genidx=$($BWEB_GENID "$COOKIE_user" "$raw_password" "$REMOTE_ADDR")
		local genid1=`echo $genidx | cut -d ' ' -f 1`
		local genid2=`echo $genidx | cut -d ' ' -f 2`
		# calculate ID for hashed password and store in positional variables
		local hashidx=$($BWEB_GENID "$COOKIE_user" "$hashed_password" "$REMOTE_ADDR")
		local hashid1=`echo $hashidx | cut -d ' ' -f 1`
		local hashid2=`echo $hashidx | cut -d ' ' -f 2`

		# correct session?
		if [ "$COOKIE_session" = "$genid1" -o "$COOKIE_session" = "$genid2" -o "$COOKIE_session" = "$hashid1" -o "$COOKIE_session" = "hashid2" ];
		then
			...
			then
				...
				hashidx=$($BWEB_GENID "$COOKIE_user" "$(_bweb_auth_calc_hash "$COOKIE_user" $POST_newpassword)" "$REMOTE_ADDR")
				hashid2=`echo $hashidx | cut -d ' ' -f 2` 
			fi

			# need to update cookie?
			if [ -n "$POST_user" -o "$COOKIE_session" != "$hashid2" ];
			then
				echo -e "Set-Cookie: user=$COOKIE_user; path=/\r"
				echo -e "Set-Cookie: session=$hashid2; path=/\r"
			fi
		else
...
_bweb_auth_check_for_wizard() {
  ...
  if [ -n "$POST_user" -a -n "$POST_password" ];
  then
    local pwenc=`echo -n $POST_password | /V2O/bin/urlencoder`
    set -- $($BWEB_GENID "$POST_user" "$(_bweb_auth_calc_hash $POST_user $pwenc)" "$REMOTE_ADDR")
    COOKIE_user="${POST_user:=dummy}"
    COOKIE_session="$2"
    if [ x"$COOKIE_session" = "x" ]; then
      COOKIE_session=$POST_password
  ...
  then
    local raw_password=$(_bweb_auth_get_password "$COOKIE_user")
    local hashed_password=$(_bweb_auth_calc_hash "$COOKIE_user" $raw_password)
    # calculate ID for raw password and store in variables
    local genidx=$($BWEB_GENID "$COOKIE_user" "$raw_password" "$REMOTE_ADDR")
    local genid1=`echo $genidx | cut -d ' ' -f 1`
    local genid2=`echo $genidx | cut -d ' ' -f 2`
  ...
  [ "$_DEBUG" == "on" ] && echo "EXIT _bweb_auth_check_for_wizard" >&2
}
...
EOF
	...
	*) echo "Logged in as <strong>$bweb_auth_user</strong> &nbsp; <a href='/cgi-bin/localui/dashboard_page_$BWEB_DEVICENAME.cgi?logout'>Logout</a>"
...
bweb_admin_password() {
	adminpwd=`/V2O/bin/get_admin | grep 'name="password"' | head -1 | cut -f4 -d'"'`
	export adminpwd
	[ "$_DEBUG" == "on" ] && echo "EXIT bweb_upgrade_url" >&2
}
...
bweb_header() {
	...
	_bweb_signature
	cat $BWEB_TEMPLATE_DIR/$BWEB_DEVICENAME/header0.html | sed  -e s/DEVICETITLE/`hostname`/ | sed -e "s/SIGNATURE/${BWEB_SIGNATURE}/"
...
bweb_footer() {
	...
	if [ "$BWEB_DEVICENAME" == "dms" ]; then
		bweb_setDeviceType
		echo "<input type='hidden' id='isdevicecombo' value='${is_combo}'>"	
	fi
...
_bweb_getupdatedconfig() {
	if [ ! -f /tmp/describe_$1.xml ]; then
		...
		if [ x"$1" = "xif_list" ]; then
				...
				#| grep -v enable | grep -v multicast | grep -v mtu | \
				#grep -v "MTU Size" /tmp/describe_$1.xml |\
				
				sed -e 's/List of Network Interfaces/Network Interface/' /tmp/describe_$1.xml \
				> /tmp/scratch
				mv /tmp/scratch /tmp/describe_$1.xml
		elif [ x"$1" = "xhost_list" ]; then
			grep -v burst /tmp/describe_$1.xml > /tmp/scratch
			sed -e 's/List of Host Entries/Host Table/'  /tmp/scratch > /tmp/describe_$1.xml
		
		elif [ x"$1" = "xnas_list" ]; then
			grep -v burst /tmp/describe_$1.xml > /tmp/scratch
			sed -e 's/NAS Server/NAS Configuration/'  /tmp/scratch > /tmp/describe_$1.xml
		
		elif [ x"$1" = "xtx_control_ports_parm" ]; then
			grep -v burst /tmp/describe_$1.xml > /tmp/scratch
			sed -e 's/Encoder Control Port Range/Control Ports Range/'  /tmp/scratch > /tmp/describe_$1.xml

		elif [ x"$1" = "xdms_server_options" ]; then
			grep -v burst /tmp/describe_$1.xml > /tmp/scratch
			sed -e 's#XMPP Server IP/Name#Management Server#'  /tmp/scratch > /tmp/describe_$1.xml
		
		elif [ x"$1" = "xdb_management" ]; then
			grep -v burst /tmp/describe_$1.xml > /tmp/scratch
			sed -e 's/Enable Export/Enable Backup on FTP/g;s/Export URL/FTP Address/g' /tmp/scratch > /tmp/describe_$1.xml

		elif [ x"$1" = "xrx_av_ports_parm" ]; then
			grep -v burst /tmp/describe_$1.xml > /tmp/scratch
			sed -e 's/Rx//'  /tmp/scratch > /tmp/describe_$1.xml
...
bweb_getlistconfig() {
	...
	elif [ x"$1" == "xif_list" ]; then
		# The entry for mtu needs to be removed from both describe_if_list and get_if_list as it is not to be displayed in the table.
		sed -i '/mtu/d' /tmp/describe_$1.xml
		sed -i '/mtu/d' /tmp/get_$1.xml
...
bweb_getconfig() {
	...
	if [ "$1" = "admin" ]; then
		cat <<FakeGetAdmin
<Group name="Get Administration Account">
  <Response>
    <Attribute name="Status" value="Success"></Attribute>
    <Attribute name="Reason" value="N/A"></Attribute>
  </Response>
  <ObjectEntry name="AdminAccount">
      <Attribute name="password" value="Oo0Oo0Oo0"></Attribute>
  </ObjectEntry>
</Group>
FakeGetAdmin
	else
...
bweb_cli() {
	...
	echo '' > /tmp/cmd.results
	#sed part has been added as a hack for the special case of admin password where we are not using any straight forward cli
	if [ "$FORM_adminpwd" = "Oo0Oo0Oo0" ]; then
		FORM_adminpwd=`/V2O/bin/get_admin | grep 'name="password"' | head -1 | cut -f4 -d'"'`
	elif [ "$FORM_password" = "Oo0Oo0Oo0" ]; then
		FORM_password=`cat /tmp/get_admin.xml | grep 'name="password"' | head -1 | cut -f4 -d'"'`
	fi
	for iv in `env | sed 's#FORM_adminpwd#FORM_cli_admin=1 \nFORM_admin_password#g' | grep FORM_cli | sed -e 's/FORM_cli_//'`
	do
		i=`echo ${iv} | sed -e 's/=.*$//'`
		v=`echo ${iv} | sed -e 's/^.*=//'`
		if [ -z $v ]; then
		...
		local envvar=`env | sed 's#FORM_adminpwd#FORM_cli_admin=1 \nFORM_admin_password#g' | sed -e 's/ /_spaces_/g'`
		
		if [ $i == "ldap_configuration" ]; then
			i="xmpp_server"
			cmdcli="/V2O/bin/set_$i"
			envvar=`echo $envvar | sed "s/ldap_configuration/$i/g"`
		elif (test "$idx" = "0")
			then
...
#		cmdcli=`echo $cmdcli | sed -e s'/set_if_list/mod_if_list 1/'`
		...
		envvar=`echo $envvar | sed -e "s/FORM_${i}_index/ /g;s/FORM_${i}_deletes/ /g"`
		cmd=""
		local x=`echo $envvar | tr ' ' '\n' |grep $i\_ |grep FORM |grep -v _cli_`
...
#						cmd="$cmd -${tmparg//=/ } "
		...
		case "$i" in
			...
			*admin*)
				...
				bweb_execute_cli "$i" "$cmdcli" "$cmd"
				echo "<clicmd name=\"save_config\">" >> /tmp/cmd.results
				echo /V2O/bin/save_config >> /tmp/cmd.log
				echo /V2O/bin/save_config | /bin/ash | tee -a /tmp/cmd.results >> /tmp/cmd.log
				echo "</clicmd>" >> /tmp/cmd.results
				;;
			*ms_server_options*)
				_bweb_cli_special_case "$i" "$cmd"
				return
				;;
			*dms_server_options*)
				_bweb_cli_special_case "$i" "$cmd"
				...
				;;
			*ha_configuration*)
				_bweb_cli_special_case_ha "$i" "$cmd"
				return
				;;
			*)
				# When a column is deleted, no other cli is to be executed alongwith. 
				# In one iteration, only one call should be placed at a time, either a delete or set or add or modify
				if [ -n "$del" ]; then
					...
					if [ "$i" == "dns_client" ]; then 
						_bweb_setDeviceName "$i" "$cmdcli" "$cmd"
					fi
				fi
				;;
			esac
...
# whenever the user sets the dns client hostname, we also need to set the xmpp agent device name with the same value
_bweb_setDeviceName() {
	i=$1
	cmdcli=$2
	cmd=$3
	
	isHostnameChanged=`echo $cmd | grep "hostname"`
		
	if [ -n "$isHostnameChanged" ]; then
		cmdcli="/V2O/bin/set_xmpp_agent"
		OldIFS=$IFS
		IFS="-"
		tempArr=$cmd
		...
		for x in $tempArr
		do
				x=`echo $x | grep hostname`

				if [ -n "$x" ]
				then
						IFS=$OldIFS
						cmd=`echo $x | sed 's/hostname/-devicename/g'`
				fi
		done
