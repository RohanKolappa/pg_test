...
#	variable won't be set at all which is equivalent to an empty string. 
	if [ "${FORM_xmpp_server_checkbox_enable}" == "off" ] && [ "${FORM_ntp_dmsipaddressforreboot}" != "" ]; then
		status="REBOOT"
		echo "<div class='result'><status>${status}</status></div>"
		/V2O/bin/reboot_system
	elif [ "${FORM_xmpp_server_checkbox_enable}" == "on" ]; then
		if [ "${FORM_xmpp_server_checkbox_enablediscovery}" != "" ] || [ "${FORM_xmpp_agent_password}" != "" ] || [ "${FORM_ntp_comboipaddressforreboot}" != "" ] || [ "${FORM_ntp_server_checkbox_enable}" != "" ]; then
			status="REBOOT"
			echo "<div class='result'><status>${status}</status></div>"
			/V2O/bin/reboot_system
		else
