  ...
  for (var name in data) {
    switch (name) {
      ...
      case 'Free':
        ...
        break;
      case 'Usable':
        if (display.memory_usable !== data[name]) {
          display.memory_usable = parseInt(data[name]);
          if (isNaN(display.memory_usable)) display.memory_usable = 0;
        }
        break;
    }
    var total = display.memory_total;
    var usable = display.memory_usable;
    var percent = Math.round(100*(total - usable)/total);
...
var barco_status_url = "/cgi-bin/localui/ngs/get_status.cgi?cmd=";
...
function barco_processCmdData(id, self) {
  ...
  if (id === "upgrade") {
	showPrompt=true;
	confirmationMsg="This utility will allow you to upgrade software on this NGS. The NGS will reboot automatically. Proceed with software upgrade?";
	...
	confirmationMsg="All settings will be lost except the network settings. If \"Reset network settings\" is also checked then the NGS will revert back to a DHCP assigned IP address that this client may not be able to reach. Do you wish to continue?";
  }
  if(showPrompt)
...
function barco_sendAJAXCmdRequest(data) {
  ...
  $.ajax({  
    type: "GET",  
    url: "/cgi-bin/localui/ngs/processcmd.cgi",
    // dataType: 'xml',
    data: data,  
