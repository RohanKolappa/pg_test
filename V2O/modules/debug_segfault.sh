...
if [ x$ip = "x" -o x$base = "x" ] ; then
    ...
    echo "libv2o.so library base offset on the command line."
    echo
    echo "For example for a segfault message such as:"
    echo "segfault at 0 ip b7689cc5 sp b50fe090 error 4 in libv2o.so[b7675000+40000]"
    echo 
...
if [ x$skip != "xskip" ] ; then
   ...
   objdump -DClS libv2o.so >| LIBV2O_DEBUG
fi
...
grep -n -A 20 -B 20 "${diff}:" LIBV2O_DEBUG

if [ x$? =  "x1" ] ; then
    echo "Sorry... could not find ${diff}: in the debug dump file LIBV2O_DEBUG"
    exit 1
fi
