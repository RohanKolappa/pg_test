...
static int i2creg_main(int argc, char **argv) {
    ...
    int opt, debug = 0, silent = 0, do_list = 0, rc = 0, hardware_type;
    ...
    if (hardware_type == XPI_DEVICE) {
        ...
        if (do_list) {
            while (reg <= MAX_I2C_REG(addr)) {
                if (do_edid_eeprom)
                    rc = hal_read_i2c_val_k(hp, I2CIO_TYPE_EDIDEEPROM_READ, 
				addr, (unsigned short) reg, 1, b);
                else if (do_mon_edid)
    ...
    do {
        ...
        if (strlen(buff) > 1) {
            ...
            b[0] = cval;
            if (do_edid_eeprom)
                rc = hal_write_i2c_val_k(hp, I2CIO_TYPE_EDIDEEPROM_WRITE, addr, 
                                        (unsigned short) reg, 1, b);
            else if (do_main_i2c) 
...
static int getdds_main(int argc, char **argv) {
    ...
    if (hp == NULL) {
        printf("Unable to communicate with hardware\n");
        return -1;
    }
    else if ((hardware_type = hal_get_hardware_type(hp)) == XPI_DEVICE) {
        printf("Hardware does not support this command.\n");
        hal_release(hp);
        return -1;
    }
...
static int readpixel_main(int argc, char **argv) {
    int opt, cno = 1, i, x = -1, y = -1, active_interface; 
    HAL *hp;
    unsigned long lval;
...
static int testcrop_main(int argc, char **argv) {
    int opt, cno = 1, xstart = 0, width = -1, ystart = 0, height = -1; 
    int disable = 0;
    HAL *hp;
...
static int testfan_main(int argc, char **argv) {
    ...
    if (get_hardware_profile() == HARDWARE_PROFILE_XP100) {
        ...
        i = 0;
        while (1) {
            cputemp = get_sysfs_attribute(TEMPERATURE_PROC_FILE, 
                    "temperature: %d C\n");
...
static int tempmon_main(int argc, char **argv) {
    ...
    while (1) {
        rc = hal_monitor_temperature(hp, TEMPMON_FLAG_PRINT);
        if (rc < 0) {
