...
if [ x$ip = "x" ] ; then
    ...
    echo "segfault at dd ip 08074ce6 sp b4d470d0 error 4 in vutils[8048000+91000]"
    echo 
...
# Create full objectdump of vutils executable
skip=$2
if [ x$skip != "xskip" ] ; then
   echo "Extracting debug info from vutils executable."
   echo "This will take a while..."
   objdump -DClS vutils >| VUTILS_DEBUG
fi
...
grep -n -A 20 -B 20 ${ip} VUTILS_DEBUG

if [ x$? =  "x1" ] ; then
    echo "Sorry... could not find ${ip} in the debug dump file VUTILS_DEBUG"
    exit 1
fi
