...
#include "rx_av_buffers_menu_gen.h"

static void paramsInitRxAVBuffers(struct rx_av_buffers_params *params)
{
    ...
    params->write_video_queue_limit_set = 1;
    params->write_video_queue_limit_value = fcqlimitio.video_write_queue_limit;
...
int get_configured_rx_queue_limits(HAL *hp, 
                                   ...
                                   int *writevideolimit, 
                                   int *writeaudiolimit) {
    struct rx_av_buffers_params params;
    params.userptr = hp;
    retrieveRxAVBuffersValues(&params);
    *readaudiolimit = params.read_audio_queue_limit_value;
    *writevideolimit = params.write_video_queue_limit_value;
    *writeaudiolimit = params.write_audio_queue_limit_value;
    return 0;
}

int applyRxAVBuffers(struct rx_av_buffers_params *params,
        struct rx_av_buffers_params *orig_params)
{
    ...
    if (params->write_video_queue_limit_set == 1) {
        fcqlimitio.video_write_queue_limit = params->write_video_queue_limit_value;
    }
...
void RxAVBuffersInit()
{

    registerRxAVBuffersParamsInitFunction
        (paramsInitRxAVBuffers);
}
